 ...
 * @returns Next task result { success: boolean, data?: any, error?: { code: string, message: string } }
 */
export async function nextTaskDirect(
	args: NextTaskArgs,
	log: Logger
): Promise<ApiResult<NextTaskResult>> {
	...
	const coreNextTaskAction = async (): Promise<ApiResult<NextTaskResult>> => {
		try {
