...
import { TASKMANAGER_TASKS_FILE } from '../../../../src/constants/paths.js';
...
interface ParsePRDArgs {
	projectRoot: string;
	input?: string;
	output?: string;
	numTasks?: string | number;
	force?: boolean;
	append?: boolean;
	research?: boolean;
}

interface ParsePRDResult {
	message: string;
	outputPath: string;
	telemetryData?: any;
}
 ...
 *
 * @param args - Command arguments containing projectRoot, input, output, numTasks options.
 ...
 */
export async function parsePRDDirect(
	args: ParsePRDArgs,
	log: Logger,
	context: SessionContext = {}
): Promise<ApiResult<ParsePRDResult>> {
	...
	const outputPath = outputArg
		...
		: resolveProjectPath(TASKMANAGER_TASKS_FILE, args) ||
			path.resolve(projectRoot, TASKMANAGER_TASKS_FILE);
	...
	try {
		...
		const result: ParsePRDCoreResult = await parsePRD(
			inputPath,
			outputPath,
			numTasks,
			{
