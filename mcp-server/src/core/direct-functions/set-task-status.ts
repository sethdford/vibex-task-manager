 ...
 */
export async function setTaskStatusDirect(
	...
	try {
		...
		try {
			...
			// Return success data
			const result: ApiResult<SetTaskStatusResult> = {
				success: true,
				data: {
			...
			if (result.data!.status === 'done') {
				try {
					...
					const nextResult: ApiResult<NextTaskData> = await nextTaskDirect(
						{
			...
			return {
				...
				error: {
					...
					message: errorMessage || 'Unknown error setting task status'
				}
			};
		} finally {
