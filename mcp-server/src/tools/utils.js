...
function getProjectRoot(projectRootRaw, log) {
    ...
    if (lastFoundProjectRoot) {
        log.info(`Using last found project root from tasks.json search: ${lastFoundProjectRoot}`);
        return lastFoundProjectRoot;
    }
    // 4. Check if current directory has any project markers
    const cwd = process.cwd();
    const hasMarkers = PROJECT_MARKERS.some(marker => fs.existsSync(path.join(cwd, marker)));
    if (hasMarkers) {
