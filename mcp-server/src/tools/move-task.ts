...
import { moveTaskDirect } from '../core/vibex-task-manager-core.js';
 ...
 */
export function registerMoveTaskTool(server: any): void {
		const tool: MCPTool = {
			...
			try {
				...
				if (fromIds.length > 1) {
					...
					for (let i = 0; i < fromIds.length; i++) {
						...
						if (!result.success) {
							log.error(
								`Failed to move ${fromId} to ${toId}: ${result.error}`
							);
						} else {
				...
				return createErrorResponse(
					`Failed to move task: ${(error as Error).message}`,
					'MOVE_TASK_ERROR'
				);
			}
		})
