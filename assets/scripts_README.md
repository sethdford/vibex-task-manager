...
Task Manager configuration is now managed through two primary methods:
...
1.  **`.taskmaster/config.json` File (Project Root - Primary)**
    ...
    - Managed using the `vibex-task-manager models --setup` command or the `models` MCP tool.
...
**Important:** Settings like `MODEL`, `MAX_TOKENS`, `TEMPERATURE`, `TASKMASTER_LOG_LEVEL`, etc., are **no longer set via `.env`**. Use `vibex-task-manager models --setup` instead.
...
2. **CLI Commands**
   ...
   vibex-task-manager [command] [options]
   ...
   Run `vibex-task-manager --help` or `node scripts/dev.js --help` to see detailed usage information.
...
vibex-task-manager update --from=4 --prompt="Refactor tasks from ID 4 onward to use Express instead of Fastify"

# Update all tasks (default from=1)
vibex-task-manager update --prompt="Add authentication to all relevant tasks"

# Specify a different tasks file
vibex-task-manager update --file=custom-tasks.json --from=5 --prompt="Change database from MongoDB to PostgreSQL"
```
...
vibex-task-manager set-status --id=4 --status=pending

# Mark a specific subtask as done
vibex-task-manager set-status --id=3.1 --status=done

# Mark multiple tasks at once
vibex-task-manager set-status --id=1,2,3 --status=done
```
...
vibex-task-manager expand --id=3 --research
...
vibex-task-manager clear-subtasks --id=3
...
- To use the research capabilities (e.g., `expand --research`), ensure you have:
  1.  Configured a model for the `research` role using `vibex-task-manager models --setup` (Perplexity models are recommended).
...
# Specify a different tasks file
vibex-task-manager validate-dependencies --file=custom-tasks.json
```
...
# Specify a different tasks file
vibex-task-manager fix-dependencies --file=custom-tasks.json
```
...
vibex-task-manager analyze-complexity --output=custom-report.json
...
vibex-task-manager expand --id=8 --num=5 --prompt="Custom prompt"
```
  ...
  "complexityAnalysis": [
    {
      ...
      "expansionCommand": "vibex-task-manager expand --id=8 --num=6 --prompt=\"Create subtasks...\" --research"
    }
...
# Specify a different tasks file
vibex-task-manager next --file=custom-tasks.json
```
...
vibex-task-manager show --id=1

# Show details for a subtask
vibex-task-manager show --id=1.2

# Specify a different tasks file
vibex-task-manager show 3 --file=custom-tasks.json
```
