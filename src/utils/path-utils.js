	...
	for (const tasksPath of possiblePaths) {
		if (fs.existsSync(tasksPath)) {
			...
			if (
				...
				logger.warn?.(
					`⚠️  DEPRECATION WARNING: Found tasks.json in legacy location '${tasksPath}'. Please migrate to the new .taskmaster directory structure. Run 'vibex-task-manager migrate' to automatically migrate your project.`
				);
			} else if (
				...
				logger.warn?.(
					`⚠️  DEPRECATION WARNING: Found tasks.json in legacy root location '${tasksPath}'. Please migrate to the new .taskmaster directory structure. Run 'vibex-task-manager migrate' to automatically migrate your project.`
				);
			}
			return tasksPath;
		}
		...
		for (const fileName of fileNames) {
			...
			if (fs.existsSync(prdPath)) {
				...
				if (location === 'scripts/' || location === '') {
					logger.warn?.(
						`⚠️  DEPRECATION WARNING: Found PRD file in legacy location '${prdPath}'. Please migrate to .taskmaster/docs/ directory. Run 'vibex-task-manager migrate' to automatically migrate your project.`
					);
				}
				return prdPath;
			}
	...
	for (const location of locations) {
		for (const fileName of fileNames) {
			...
			if (fs.existsSync(reportPath)) {
				...
				if (location === 'scripts/' || location === '') {
					logger.warn?.(
						`⚠️  DEPRECATION WARNING: Found complexity report in legacy location '${reportPath}'. Please migrate to .taskmaster/reports/ directory. Run 'vibex-task-manager migrate' to automatically migrate your project.`
					);
				}
				return reportPath;
			}
	...
	for (const configPath of possiblePaths) {
		if (fs.existsSync(configPath)) {
			...
			if (configPath?.endsWith(LEGACY_CONFIG_FILE)) {
				logger.warn?.(
					`⚠️  DEPRECATION WARNING: Found configuration in legacy location '${configPath}'. Please migrate to .taskmaster/config.json. Run 'vibex-task-manager migrate' to automatically migrate your project.`
				);
			}
			return configPath;
		}
