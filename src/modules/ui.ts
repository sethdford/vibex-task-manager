...
import type { Task, ComplexityReport, FindNextTaskResult } from '../types/index.js';
 ...
 */
export function succeedLoadingIndicator(spinner: Ora | null, message: string | null = null): void {
 ...
 */
export function failLoadingIndicator(spinner: Ora | null, message: string | null = null): void {
 ...
 */
export function warnLoadingIndicator(spinner: Ora | null, message: string | null = null): void {
 ...
 */
export function infoLoadingIndicator(spinner: Ora | null, message: string | null = null): void {
  ...
  if (statusBreakdown && empty > 0) {
    ...
    if (totalRemaining <= 0) {
      ...
      for (const [status, percentage] of Object.entries(statusBreakdown)) {
        ...
        const statusChars = Math.round(((percentage || 0) / totalRemaining) * empty);
 ...
 */
export function getStatusWithColor(status: string, forTable: boolean = false): string {
  if (!status) {
    return chalk.gray('❓ unknown');
  }

  const statusConfig: Record<string, { color: typeof chalk; icon: string; tableIcon: string }> = {
...
export function displayHelp(): void {
  ...
  commandCategories.forEach((category) => {
    console.log(
      boxen(chalk[category.color as 'blue' | 'cyan' | 'green' | 'yellow' | 'magenta'].bold(category.title), {
...
export async function displayNextTask(
  ...
  const nextTask: FindNextTaskResult | null = findNextTask(data.tasks, complexityReport as any) as FindNextTaskResult | null;
...
export async function displayTaskById(
  ...
  if ((task as any).testStrategy && (task as any).testStrategy.trim().length > 0) {
    console.log(
