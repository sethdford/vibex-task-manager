...
export function getResearchTemperature(
	explicitRoot: string | null = null
): number {
	// Directly return value from config
...
export function getFallbackMaxTokens(
	explicitRoot: string | null = null
): number {
	// Directly return value from config
...
export function getFallbackTemperature(
	explicitRoot: string | null = null
): number {
	// Directly return value from config
 ...
 */
export function getMcpApiKeyStatus(
	providerName: string,
	projectRoot: string | null = null
): boolean {
	...
	try {
		...
		console.error(
			chalk.red(
				`Error reading or parsing .cursor/mcp.json: ${(error as Error).message}`
			)
		);
		return false;
	}
	...
	for (const [provider, models] of Object.entries(MODEL_MAP)) {
		if (models.length > 0) {
			models.forEach((modelObj) => {
				...
				if (modelId === 'claude-sonnet-4-20250514') name = 'Claude 3.7 Sonnet';
...
export function isConfigFilePresent(
	...
	return (
		findConfigPath(null, { projectRoot: explicitRoot || undefined }) !== null
	);
}
