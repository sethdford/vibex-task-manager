...
import { AvailableModel, TelemetryData, ApiKeyStatusReport, ConfigData, TasksData, ComplexityReport } from '../types/index.js';
import { getDefaultSubtasks } from './config-manager.js';
import { TASKMANAGER_CONFIG_FILE, TASKMANAGER_TASKS_FILE } from '../constants/paths.js';
 ...
 */
export async function confirmTaskOverwrite(_tasksPath: string): Promise<boolean> {
  console.log(
    boxen(
      chalk.yellow(
 ...
 */
export function formatSweScoreWithTertileStars(score: number | null, allModels: AvailableModel[]): string {
 ...
 */
export function displayModelConfiguration(configData: ConfigData, allAvailableModels: AvailableModel[] = []): void {
  ...
  activeTable.push([
    ...
    active.main.modelId,
    formatSweScoreWithTertileStars(active.main.sweScore ?? null, allAvailableModels),
 ...
 */
export function displayAvailableModels(availableModels: AvailableModel[]): void {
 ...
 */
export function displayAiUsageSummary(telemetryData: TelemetryData, outputType: string = 'cli'): void {
  if (
 ...
 */
export async function displayComplexityReport(reportPath: string): Promise<void> {
  ...
  metaTable.push(
    ...
    [chalk.cyan.bold('Threshold Score:'), report.meta.thresholdScore.toString()],
