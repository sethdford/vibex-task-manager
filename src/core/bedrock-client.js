...
- All required fields must be present`;
        ...
        try {
            ...
            throw new BedrockClientError(`Failed to parse JSON response: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`, 'JSON_PARSE_ERROR');
        }
    }
    /**
    ...
    async streamText(options) {
        ...
        try {
            ...
            const bedrockError = error instanceof BedrockClientError
                ? error
                : new BedrockClientError(`Streaming error: ${error instanceof Error ? error.message : 'Unknown error'}`, 'STREAMING_ERROR');
        ...
        for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
            try {
                ...
                if (this.isNonRetryableError(error)) {
                    throw new BedrockClientError(`${operationName} failed: ${lastError.message}`, 'NON_RETRYABLE_ERROR', undefined, undefined);
                }
                // If this was the last attempt, throw the error
                if (attempt === this.config.maxRetries) {
                    throw new BedrockClientError(`${operationName} failed after ${this.config.maxRetries} attempts: ${lastError.message}`, 'MAX_RETRIES_EXCEEDED', undefined, undefined);
                }
