...
export function getAllBedrockModels(): Array<
	[BedrockModelId, BedrockModelInfo]
> {
	return Object.entries(BEDROCK_MODELS) as Array<
		[BedrockModelId, BedrockModelInfo]
	>;
}

export function findModelByBedrockId(
	bedrockId: string
): [BedrockModelId, BedrockModelInfo] | undefined {
...
export function getBestModelForBudget(
	maxCostPer1KTokens: number
): BedrockModelId | null {
	const affordableModels = Object.entries(BEDROCK_MODELS)
		.filter(
			([_, model]) =>
				model.outputCostPer1K <= maxCostPer1KTokens && model.isClaude
		)
...
export function isValidBedrockModel(
	modelId: string
): modelId is BedrockModelId {
...
export function getBestModelForTaskCapability(
	...
	const models = Object.entries(BEDROCK_MODELS) as Array<
		[BedrockModelId, BedrockModelInfo]
	>;

	// Filter models by capability
	const capableModels = models.filter(
		([_, model]) => model.taskCapabilities?.[capability] === true
	);
	...
	if (maxCostPer1KTokens !== undefined) {
		const affordableModels = capableModels.filter(
			([_, model]) => model.outputCostPer1K <= maxCostPer1KTokens
		);
