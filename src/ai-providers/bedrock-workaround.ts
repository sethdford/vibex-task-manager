 ...
 */
export async function generateObjectWorkaround(client: any, params: WorkaroundParams): Promise<WorkaroundResult> {
 ...
 */
function zodSchemaToDescription(schema: z.ZodSchema): string {
    if (!(schema as any)._def) return 'Unknown schema';
    ...
    if (def.typeName === 'ZodObject') {
        ...
        for (const [key, value] of Object.entries(shape)) {
            obj[key] = getFieldDescription(value as z.ZodSchema);
        }
...
function getFieldDescription(field: z.ZodSchema): string {
    if (!(field as any)._def) return 'any';
 ...
 */
function attemptSchemaFix(obj: any, schema: z.ZodSchema, error: z.ZodError): any {
