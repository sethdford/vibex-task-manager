...
export class BedrockAIProvider extends BaseAIProvider {
	 ...
	 */
	validateAuth(params: GenerateTextParams | GenerateObjectParams): void {
	 ...
	 */
	getClient(params: GenerateTextParams | GenerateObjectParams): any {
		try {
			const {
			...
			this.handleError('client initialization', error as Error);
		}
	}
	 ...
	 */
	async generateObject(params: GenerateObjectParams): Promise<ObjectResult> {
		try {
			...
			log(
				'debug',
				`Generating ${this.name} object with workaround for model: ${params.modelId}`
			);
			...
			log(
				'debug',
				`${this.name} generateObject (direct API) completed successfully for model: ${params.modelId}`
			);
	 ...
	 */
	async generateText(params: GenerateTextParams): Promise<AIResult> {
		try {
			this.validateParams(params);
			this.validateMessages(params.messages);
			...
			log(
				'debug',
				`Generating ${this.name} text with direct API for model: ${params.modelId}`
			);
			...
			log(
				'debug',
				`${this.name} generateText (direct API) completed successfully for model: ${params.modelId}`
			);
