...
import {
	getBedrockModel,
	getBestModelForTaskCapability,
	BedrockModelInfo,
	BEDROCK_MODELS
} from '../core/bedrock-models.js';
import {
	Task,
	Subtask,
	TaskCreateInput,
...
export class TaskService implements ITaskService {
	...
	private async initializeModel() {
		...
		this.defaultModel =
			(config.models?.main?.modelId as ClaudeModelId) || 'claude-instant-v1';
	}

	private async getModelForOperation(
		operation: 'canGenerateSubtasks' | 'canAnalyzeComplexity' | 'canParsePRD'
	): Promise<ClaudeModelId> {
		const config = await this.configService.getConfig();
		...
		if (configuredModel) {
			...
			if (!modelInfo.taskCapabilities?.[operation]) {
				...
				const supportingModels = Object.entries(BEDROCK_MODELS)
					.filter(
						([_, model]: [string, BedrockModelInfo]) =>
							model.taskCapabilities?.[operation]
					)
					.map(
						([id, model]: [string, BedrockModelInfo]) => `${id} (${model.name})`
					);
	...
	async getTasks(options: TaskQueryOptions = {}): Promise<TaskQueryResult> {
		...
		return {
			...
			hasMore: options.pagination
				? options.pagination.page * options.pagination.limit < totalCount
				: false
		};
	}
	...
	async createTaskFromPrompt(
		...
		try {
			...
			throw new AIServiceError(
				'Failed to parse AI response for task creation.',
				JSON.stringify({ prompt, response: response.text })
			);
		}
	...
	async addSubtask(
		taskId: number,
		subtaskInput: SubtaskCreateInput
	): Promise<Subtask> {
		...
		for (const task of tasks) {
			...
			if (chainDepth > 5) {
				warnings.push({
					...
					suggestion:
						'Consider breaking down dependencies or restructuring tasks'
				});
			}
		}
	...
	async expandTask(
		taskId: number,
		options: { maxSubtasks?: number; detailLevel?: string } = {}
	): Promise<Subtask[]> {
		...
		const prompt = this.buildTaskExpansionPrompt(
			task,
			maxSubtasks,
		...
		try {
			...
			const SubtaskArraySchema = z.array(
				z.object({
	...
	async getNextTask(criteria: NextTaskCriteria = {}): Promise<NextTaskResult> {
		const validatedCriteria =
			ValidationSchemas.NextTaskCriteria.parse(criteria);
		const tasksData = await this.loadTasksData();
		...
		const availableTasks = tasksData.tasks.filter((task) => {
			...
			// Check status
			if (!validatedCriteria.includeInProgress && task.status === 'in-progress')
				return false;
			if (task.status === 'done' || task.status === 'cancelled') return false;

			// Check assignee
			if (
				validatedCriteria.assignee &&
				task.assignee !== validatedCriteria.assignee
			)
				return false;

			// Check max complexity
			if (
				validatedCriteria.maxComplexity &&
				task.complexity &&
				task.complexity > validatedCriteria.maxComplexity
			) {
				return false;
			}
	...
	async moveTask(taskId: number, newPosition: number): Promise<Task[]> {
		...
		const insertIndex = Math.max(
			0,
			Math.min(newPosition, tasksData.tasks.length)
		);
	...
	private applyFilters(tasks: Task[], filter: TaskFilter): Task[] {
		return tasks.filter((task) => {
			...
			// Depends on filter
			if (filter.dependsOn && !task.dependencies.includes(filter.dependsOn))
				return false;
			...
			if (filter.search) {
				...
				const matchesDetails =
					task.details?.toLowerCase().includes(searchTerm) || false;
	...
	private detectCycle(
		...
		const dependencies =
			overrideDependencies || currentTask?.dependencies || [];
	...
	private calculateDependencyDepth(
		...
		const maxDepth = Math.max(
			...task.dependencies.map((depId) =>
				this.calculateDependencyDepth(depId, tasks, new Set(visited))
			)
		);
	...
	private calculateTaskScore(task: Task, criteria: NextTaskCriteria): number {
		...
		if (criteria.dueDateWeight > 0 && task.dueDate) {
			...
			const dueDateScore =
				daysUntilDue <= 0 ? 1 : Math.max(0, 1 - daysUntilDue / 30); // Urgent tasks score higher
			score += dueDateScore * criteria.dueDateWeight;
		}

		// Preferred tags bonus
		if (criteria.preferredTags.length > 0 && task.tags) {
			const hasPreferredTag = criteria.preferredTags.some((tag) =>
	...
	private generateTaskReasoning(
		...
		if (task.dependencies.length === 0)
			reasoning.push('No blocking dependencies');
		if (task.complexity && task.complexity <= 3)
			reasoning.push('Low complexity, quick to complete');
		if (task.dueDate) {
			const daysUntilDue =
				(new Date(task.dueDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24);
	...
	private calculateTaskFactors(task: Task, criteria: NextTaskCriteria) {
		...
		return {
			...
			dependencies:
				Math.max(0, 1 - task.dependencies.length * 0.1) *
				criteria.dependencyWeight,
			complexity: task.complexity
				? Math.max(0, 1 - (task.complexity - 1) / 9) * criteria.complexityWeight
				: 0,
	...
	private generateProjectRecommendations(
		...
		if (blockedTasks.length > allTasks.length * 0.3) {
			recommendations.push(
				'Consider reviewing and completing blocking tasks to unblock the project'
			);
		}

		const highPriorityBlocked = blockedTasks.filter(
			(bt) => bt.task.priority === 'high'
		).length;
		if (highPriorityBlocked > 0) {
			recommendations.push(
				`${highPriorityBlocked} high-priority tasks are blocked - prioritize their dependencies`
			);
		}
...
Respond with a JSON object with 'projectName', 'overview', 'estimatedComplexity', 'estimatedDuration', and a 'tasks' array. Each task object should have 'title', 'description', 'priority', 'complexity', and 'estimatedHours'.`;
	...
	async enableSparc(taskId: number): Promise<{ task: Task; message: string }> {
		const task = await this.sparcService.enableSparc(
			taskId,
			(id) => this.getTask(id),
		...
		return {
			task,
			message: `SPARC enabled for task #${taskId}. Current phase: ${task.sparc?.currentPhase}`
		};
	}

	async disableSparc(taskId: number): Promise<{ task: Task; message: string }> {
		const task = await this.sparcService.disableSparc(
			taskId,
			(id) => this.getTask(id),
			(id, updates) => this.updateTask(id, updates)
		);
	...
	async advanceSparcPhase(
		taskId: number,
		phase: SparcStatus
	): Promise<{ task: Task; message: string }> {
		const task = await this.sparcService.advancePhase(
			taskId,
			phase,
			(id) => this.getTask(id),
			(id, updates) => this.updateTask(id, updates)
	...
	async generateSparcRequirements(taskId: number): Promise<any> {
		return this.sparcService.generateRequirements(
			taskId,
			(id) => this.getTask(id),
			(id, updates) => this.updateTask(id, updates)
		);
	...
	async generateSparcPseudocode(taskId: number): Promise<any> {
		return this.sparcService.generatePseudocode(
			taskId,
			(id) => this.getTask(id),
			(id, updates) => this.updateTask(id, updates)
		);
	...
	async generateSparcArchitecture(taskId: number): Promise<any> {
		return this.sparcService.generateArchitecture(
			taskId,
			(id) => this.getTask(id),
			(id, updates) => this.updateTask(id, updates)
		);
	...
	async generateSparcTests(taskId: number): Promise<any> {
		return this.sparcService.generateTests(
			taskId,
			(id) => this.getTask(id),
			(id, updates) => this.updateTask(id, updates)
		);
	...
	async validateSparcCompletion(
		...
		const validationResult = await this.sparcService.validateCompletion(
			taskId,
			(id) => this.getTask(id),
			(id, updates) => this.updateTask(id, updates)
		);
		if (validationResult.success) {
			return {
				success: true,
				message: `SPARC validation successful for task #${taskId}.`,
				report: validationResult.report
			};
		}
		return {
			success: false,
			message: `SPARC validation failed for task #${taskId}.`,
			report: validationResult.report
		};
	}
}
