...
export class ConfigService implements IConfigService {
	...
	async autoConfigureModels(
		...
		const detectionResult = await autoDetect.detectModels(
			options.testAccess || false
		);
		...
		if (detectionResult.available.length === 0) {
			throw new ConfigurationError(
				detectionResult.error ||
					'No accessible Claude models found in your AWS Bedrock region.',
				'no_models'
			);
		}
		...
		config.models.main = {
			...
			maxTokens: Math.min(
				config.models.main.maxTokens,
				CLAUDE_MODELS[recommendations.main].maxTokens
			)
		};
		...
		if (recommendations.research) {
			config.models.research = {
				...
				maxTokens: Math.min(
					config.models.research.maxTokens,
					CLAUDE_MODELS[recommendations.research].maxTokens
				)
			};
		} else {
			...
			warnings.push(
				'No separate research model available, using main model for research tasks'
			);
		}
		...
		if (recommendations.fallback && config.models.fallback) {
			config.models.fallback = {
				...
				maxTokens: Math.min(
					config.models.fallback.maxTokens,
					CLAUDE_MODELS[recommendations.fallback].maxTokens
				)
			};
			...
			warnings.push(
				'No separate fallback model available, using main model as fallback'
			);
		}
	...
	async setupConfiguration(options: {
		...
		if (options.mainModel) {
			...
			config.models.main.maxTokens = Math.min(
				config.models.main.maxTokens,
				mainModelInfo.maxTokens
			);
		}
		if (options.researchModel) {
			...
			config.models.research.maxTokens = Math.min(
				config.models.research.maxTokens,
				researchModelInfo.maxTokens
			);
		}
		if (options.fallbackModel && config.models.fallback) {
			...
			config.models.fallback.maxTokens = Math.min(
				config.models.fallback.maxTokens,
				fallbackModelInfo.maxTokens
			);
		}
	...
	): Promise<BedrockModelConfig> {
		const config = await this.getConfig();
		const modelConfig = config.models[modelType];

		if (!modelConfig) {
			throw new ConfigurationError(
				`Model configuration for ${modelType} not found`
			);
		}
	...
	async getAwsCredentials(
		modelType: 'main' | 'research' | 'fallback' = 'main'
	): Promise<{
		region: string;
		profile?: string;
		accessKeyId?: string;
	...
	async isConfigurationValid(): Promise<{
		...
		try {
			...
			if (!mainModelValid) {
				errors.push(
					'Cannot connect to main model - check AWS credentials and permissions'
				);
			}

			if (config.models.research.modelId) {
				const researchModelValid = await this.testModelConfig(
					config.models.research
				);
				if (!researchModelValid) {
					warnings.push(
						'Cannot connect to research model - will fallback to main model'
					);
				}
			}

			if (config.models.fallback) {
				const fallbackModelValid = await this.testModelConfig(
					config.models.fallback
				);
	...
	async exportConfig(): Promise<string> {
		...
		const exportConfig = {
			...
			models: {
				...
				research: {
					...config.models.research,
					accessKeyId: config.models.research.accessKeyId
						? '[REDACTED]'
						: undefined,
					secretAccessKey: config.models.research.secretAccessKey
						? '[REDACTED]'
						: undefined,
					sessionToken: config.models.research.sessionToken
						? '[REDACTED]'
						: undefined
				},
				fallback: config.models.fallback
					? {
							...config.models.fallback,
							accessKeyId: config.models.fallback.accessKeyId
								? '[REDACTED]'
								: undefined,
							secretAccessKey: config.models.fallback.secretAccessKey
								? '[REDACTED]'
								: undefined,
							sessionToken: config.models.fallback.sessionToken
								? '[REDACTED]'
								: undefined
						}
					: undefined
	...
	private async validateModelConfigs(config: Config): Promise<void> {
		...
		if (
			config.models.fallback &&
			!BedrockClient.validateModel(config.models.fallback.modelId)
		) {
			throw new ConfigurationError(
