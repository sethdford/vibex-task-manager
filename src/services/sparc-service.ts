...
export class SparcService {
	...
	async advancePhase(
		...
		const currentPhaseIndex = phaseOrder.indexOf(
			task.sparc.currentPhase || 'specification'
		);
		const targetPhaseIndex = phaseOrder.indexOf(targetPhase);

		if (targetPhaseIndex < currentPhaseIndex) {
			throw new Error(
				`Cannot move backwards in SPARC phases. Current: ${task.sparc.currentPhase}, Target: ${targetPhase}`
			);
		}
	...
	async getProgress(taskId: number, getTask: TaskGetter): Promise<any> {
		...
		const completedPhases = Object.values(task.sparc.phases).filter(
			(p) => p.status === 'done'
		).length;
	...
	async generateRequirements(
		...
		const requirements =
			parsed?.requirements || this.extractRequirementsFromText(response.text);
	...
	async generatePseudocode(
		...
		const parsed = this.parseJsonResponse(
			response.text,
			z.object({ agentCoordination: z.string(), taskFlow: z.string() })
		);

		if (parsed) {
			const updatedSparc = { ...task.sparc! };
			updatedSparc.phases.pseudocode.agentCoordination =
				parsed.agentCoordination;
	...
	async generateTests(
		...
		const parsed = this.parseJsonResponse(
			response.text,
			z.object({ testCases: z.array(z.string()) })
		);
		const testCases =
			parsed?.testCases || this.extractTestCasesFromText(response.text);
	...
	async validateCompletion(
		...
		for (const phaseName of phaseOrder) {
			const phase =
				task.sparc.phases[phaseName as keyof typeof task.sparc.phases];
			if (phase?.status !== 'done') {
				issues.push(
					`Phase '${phaseName}' is not complete. Current status: ${phase?.status}`
				);
			}
	 ...
	 */
	getSparcProgress(task: Task): {
		currentPhase: SparcStatus;
		progress: number;
