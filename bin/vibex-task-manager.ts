...
function createDevScriptAction(commandName: string): (options: any, cmd: any) => void {
	return (options: any, cmd: any) => {
		...
		if (camelCaseFlags.length > 0) {
			...
			console.error(
				'\nExample: vibex-task-manager parse-prd --num-tasks=5 instead of --numTasks=5\n'
			);
			process.exit(1);
		}
		...
		const positionals = new Set<string>(); // Track positional args we've seen
		...
		const userOptions = new Set<string>();
...
process.on('uncaughtException', (err: Error) => {
	...
	if ((err as any).code === 'commander.unknownOption') {
		...
		console.error(
			chalk.yellow(
				`Run 'vibex-task-manager ${command} --help' to see available options for this command`
			)
		);
		process.exit(1);
	}
	...
	if ((err as any).code === 'commander.unknownCommand') {
		const command = err.message.match(/'([^']+)'/)?.[1];

		console.error(chalk.red(`Error: Unknown command '${command}'`));
		console.error(
			chalk.yellow(`Run 'vibex-task-manager --help' to see available commands`)
		);
		process.exit(1);
	}
...
// Show help if no command was provided (just 'vibex-task-manager' with no args)
