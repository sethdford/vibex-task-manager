...
# --- Configuration ---
# Assumes script is run from the project root (vibex-task-manager)
VIBEX_SOURCE_DIR="." # Current directory is the source
# Base directory for test runs, relative to project root
BASE_TEST_DIR="$VIBEX_SOURCE_DIR/tests/e2e/_runs"
# Log directory, relative to project root
LOG_DIR="$VIBEX_SOURCE_DIR/tests/e2e/log"
# Path to the sample PRD, relative to project root
SAMPLE_PRD_SOURCE="$VIBEX_SOURCE_DIR/tests/fixtures/sample-prd.txt"
# Path to the main .env file in the source directory
MAIN_ENV_FILE="$VIBEX_SOURCE_DIR/.env"
# ---

# <<< Source the helper script >>>
# shellcheck source=tests/e2e/e2e_helpers.sh
source "$VIBEX_SOURCE_DIR/tests/e2e/e2e_helpers.sh"
...
if [ "$#" -ge 1 ] && [ "$1" == "--analyze-log" ]; then
  ...
  if [ "$#" -ge 2 ] && [ -n "$2" ]; then
    ...
    ABS_LOG_DIR="$(cd "$VIBEX_SOURCE_DIR/$LOG_DIR" && pwd)"
  ...
  EXPECTED_RUN_DIR="$VIBEX_SOURCE_DIR/tests/e2e/_runs/run_$timestamp_match"
  # Make it absolute
  EXPECTED_RUN_DIR_ABS="$(cd "$VIBEX_SOURCE_DIR" && pwd)/tests/e2e/_runs/run_$timestamp_match"
  ...
  analyze_log_with_llm "$LOG_TO_ANALYZE" "$(cd "$ORIGINAL_DIR/$VIBEX_SOURCE_DIR" && pwd)" # Pass absolute project root
  ANALYSIS_EXIT_CODE=$?
...
# Wrap the main part of the script in braces and pipe its output (stdout and stderr) to tee
{
  ...
  log_step "Creating global npm link for vibex-task-manager-ai"
  if npm link; then
  ...
  log_step "Linking vibex-task-manager package locally"
  npm link vibex-task-manager
  log_success "Package linked locally."

  log_step "Initializing Task Manager project (non-interactive)"
  vibex-task-manager init -y --name="E2E Test $TIMESTAMP" --description="Automated E2E test run"
  if [ ! -f ".taskmanager/config.json" ]; then
    log_error "Initialization failed: .taskmanager/config.json not found."
    exit 1
  fi
  log_success "Project initialized."

  log_step "Parsing PRD"
  cmd_output_prd=$(vibex-task-manager parse-prd ./prd.txt --force 2>&1)
  exit_status_prd=$?
  echo "$cmd_output_prd"
  ...
  cmd_output_analyze=$(vibex-task-manager analyze-complexity --research --output complexity_results.json 2>&1)
  exit_status_analyze=$?
  ...
  vibex-task-manager complexity-report --file complexity_results.json > complexity_report_formatted.log
  ...
  cmd_output_expand1=$(vibex-task-manager expand --id=1 2>&1)
  exit_status_expand1=$?
  ...
  vibex-task-manager models --set-fallback claude-3-5-sonnet-20241022
  log_success "Set fallback model."

  log_step "Checking final model configuration"
  vibex-task-manager models > models_final_config.log
  ...
  for i in "${!providers[@]}"; do
    ...
    set_model_cmd="vibex-task-manager models --set-main \"$model\" $flag"
    ...
    add_task_cmd_output=$(vibex-task-manager add-task --prompt "$add_task_prompt" 2>&1 | tee "$add_task_output_file")
    ...
    if [ "$new_task_id" != "FAILED" ] && [ "$new_task_id" != "UNKNOWN_ID_EXTRACTION_FAILED" ]; then
      ...
      if vibex-task-manager show "$new_task_id" > "$show_output_file"; then
  ...
  vibex-task-manager list --with-subtasks > task_list_after_multi_add.log
  ...
  vibex-task-manager add-dependency --id=1 --depends-on=999 || log_error "Failed to add non-existent dependency (unexpected)"
  ...
  vibex-task-manager validate-dependencies > validate_deps_non_existent.log 2>&1 || true # Allow command to fail without exiting script
  ...
  vibex-task-manager fix-dependencies > fix_deps_after_non_existent.log
  log_success "Attempted to fix dependencies."

  log_step "Validating dependencies (after fix)"
  vibex-task-manager validate-dependencies > validate_deps_after_fix_non_existent.log 2>&1 || true # Allow potential failure
  ...
  vibex-task-manager add-dependency --id=4 --depends-on=5 || log_error "Failed to add dependency 4->5"
  vibex-task-manager add-dependency --id=5 --depends-on=4 || log_error "Failed to add dependency 5->4"
  log_success "Attempted to add dependencies 4 -> 5 and 5 -> 4."


  log_step "Validating dependencies (expecting circular error)"
  vibex-task-manager validate-dependencies > validate_deps_circular.log 2>&1 || true # Allow command to fail
  ...
  vibex-task-manager fix-dependencies > fix_deps_after_circular.log
  log_success "Attempted to fix dependencies."

  log_step "Validating dependencies (after fix circular)"
  vibex-task-manager validate-dependencies > validate_deps_after_fix_circular.log 2>&1 || true # Allow potential failure
  ...
  vibex-task-manager add-task --title="Manual E2E Task" --description="Add basic health check endpoint" --priority=low --dependencies=3 # Depends on backend setup
  log_success "Added Task $manual_task_id manually."

  log_step "Adding Task $ai_task_id (AI)"
  cmd_output_add_ai=$(vibex-task-manager add-task --prompt="Implement basic UI styling using CSS variables for colors and spacing" --priority=medium --dependencies=1 2>&1)
  exit_status_add_ai=$?
  ...
  cmd_output_update_task3=$(vibex-task-manager update-task --id=3 --prompt="Update backend server setup: Ensure CORS is configured to allow requests from the frontend origin." 2>&1)
  ...
  cmd_output_update_from5=$(vibex-task-manager update --from=5 --prompt="Refactor the backend storage module to use a simple JSON file (storage.json) instead of an in-memory object for persistence. Update relevant tasks." 2>&1)
  ...
  cmd_output_expand8=$(vibex-task-manager expand --id=8 2>&1)
  exit_status_expand8=$?
  ...
  cmd_output_update_subtask81=$(vibex-task-manager update-subtask --id=8.1 --prompt="Implementation note: Remember to handle potential API errors and display a user-friendly message." 2>&1)
  ...
  vibex-task-manager add-subtask --parent=2 --title="Subtask 2.1 for removal"
  vibex-task-manager add-subtask --parent=2 --title="Subtask 2.2 for removal"
  log_success "Added subtasks 2.1 and 2.2."

  log_step "Removing Subtasks 2.1 and 2.2 (multi-ID)"
  vibex-task-manager remove-subtask --id=2.1,2.2
  ...
  vibex-task-manager next > next_task_after_change_core.log
  ...
  vibex-task-manager list --status=done > task_list_status_done.log
  ...
  vibex-task-manager clear-subtasks --id=8
  ...
  vibex-task-manager remove-task --id="$manual_task_id,$ai_task_id" -y
  ...
  vibex-task-manager expand --id=2 # Expand task 2: Backend setup
  log_success "Attempted to expand Task 2."

  log_step "Listing tasks with subtasks (Before Clear All)"
  vibex-task-manager list --with-subtasks > task_list_before_clear_all.log
  ...
  vibex-task-manager list --with-subtasks > task_list_after_clear_all.log
  ...
  vibex-task-manager expand --id=1
  ...
  vibex-task-manager add-dependency --id=3 --depends-on=1.1
  log_success "Added dependency 3 -> 1.1."

  log_step "Showing Task 3 details (after adding subtask dependency)"
  vibex-task-manager show 3 > task_3_details_after_dep_add.log
  ...
  vibex-task-manager remove-dependency --id=3 --depends-on=1.1
  log_success "Removed dependency 3 -> 1.1."

  log_step "Showing Task 3 details (after removing subtask dependency)"
  vibex-task-manager show 3 > task_3_details_after_dep_remove.log
  ...
  cmd_output_analyze_final=$(vibex-task-manager analyze-complexity --research --output complexity_results_final.json 2>&1)
  ...
  vibex-task-manager complexity-report --file complexity_results_final.json > complexity_report_formatted_final.log
  log_success "Final Formatted complexity report saved."

  # === End AI Commands Re-test ===

  log_step "Listing tasks again (final)"
  vibex-task-manager list --with-subtasks > task_list_final.log
...
if [ -d "$TEST_RUN_DIR" ]; then
  ...
  VIBEX_SOURCE_DIR_ABS=${VIBEX_SOURCE_DIR_ABS:-$(cd "$ORIGINAL_DIR/$VIBEX_SOURCE_DIR" && pwd)}

  cd "$TEST_RUN_DIR"
  # Pass the absolute source directory path
  analyze_log_with_llm "$LOG_FILE" "$VIBEX_SOURCE_DIR_ABS"
