...
describe('Config Manager Basic Tests', () => {
  ...
  test('provider validation logic', () => {
    ...
    const validProviders = ['anthropic', 'openai', 'bedrock', 'google', 'perplexity', 'ollama', 'openrouter', 'mistral', 'xai', 'azure'];
  ...
  test('API key validation patterns', () => {
    ...
    for (const placeholder of placeholderPatterns) {
      expect(placeholder.includes('_HERE') || placeholder.includes('YOUR_')).toBe(true);
    }
