...
function createProjectStructure(addAliases, dryRun, options) {
    ...
    if (!isSilentMode() && !dryRun && !options?.yes) {
        console.log(boxen(chalk.cyan('Configuring AI Models...'), {
            padding: 0.5,
        ...
        log('info', 'Default AI models will be used. You can configure different models later using "vibex-task-manager models --setup" or "vibex-task-manager models --set-..." commands.');
    }
    // ====================================
...
function setupMCPConfiguration(targetDir) {
    ...
    if (fs.existsSync(mcpJsonPath)) {
        ...
        try {
            ...
            const hasMCPString = Object.values(mcpConfig.mcpServers).some((server) => server.args &&
                server.args.some((arg) => typeof arg === 'string' && arg.includes('vibex-task-manager-ai')));
            if (hasMCPString) {
                log('info', 'Found existing vibex-task-manager-ai MCP configuration in mcp.json, leaving untouched');
            ...
            if (!mcpConfig.mcpServers['vibex-task-manager-ai']) {
                ...
                log('info', 'Added vibex-task-manager-ai server to existing MCP configuration');
            }
            else {
                log('info', 'vibex-task-manager-ai server already configured in mcp.json');
            }
    ...
    log('info', 'MCP server will use the installed vibex-task-manager-ai package');
}
