...
import { EXAMPLE_PRD_FILE, TASKMANAGER_CONFIG_FILE, TASKMANAGER_TEMPLATES_DIR, TASKMANAGER_DIR, TASKMANAGER_TASKS_DIR, TASKMANAGER_DOCS_DIR, TASKMANAGER_REPORTS_DIR, ENV_EXAMPLE_FILE, GITIGNORE_FILE } from '../src/constants/paths.js';
...
const LOG_LEVEL = process.env.TASKMANAGER_LOG_LEVEL
    ? LOG_LEVELS[process.env.TASKMANAGER_LOG_LEVEL.toLowerCase()]
...
`;
        ...
        log('error', `Failed to add aliases: ${error instanceof Error ? error.message : String(error)}`);
        return false;
    }
}
...
function copyTemplateFile(templateName, targetPath, replacements = {}) {
    ...
    switch (templateName) {
        ...
        case 'taskmanager.mdc':
            sourcePath = path.join(__dirname, '..', '.cursor', 'rules', 'taskmanager.mdc');
            break;
    ...
    if (fs.existsSync(targetPath)) {
        ...
        if (filename === 'README-vibex-task-manager.md') {
            ...
            const taskMasterReadmePath = path.join(path.dirname(targetPath), 'README-vibex-task-manager.md');
            fs.writeFileSync(taskMasterReadmePath, content);
            log('success', `Created ${taskMasterReadmePath} (preserved original README-vibex-task-manager.md)`);
            return;
        }
...
async function initializeProject(options = {}) {
    ...
    if (skipPrompts) {
        ...
        const projectDescription = options.description || 'A project managed with Task Manager AI';
        ...
        try {
            ...
            const addAliasesInput = await promptQuestion(rl, chalk.cyan('Add shell aliases for vibex-task-manager? This lets you type "tm" instead of "vibex-task-manager" (Y/n): '));
            const addAliasesPrompted = addAliasesInput.trim().toLowerCase() !== 'n';
            // Confirm settings...
            console.log('\nTask Manager Project settings:');
            console.log(chalk.blue('Add shell aliases (so you can use "tm" instead of "vibex-task-manager"):'), chalk.white(addAliasesPrompted ? 'Yes' : 'No'));
            ...
            rl.close();
            log('error', `Error during initialization process: ${error instanceof Error ? error.message : String(error)}`);
            process.exit(1);
        }
    }
}
...
function createProjectStructure(addAliases, dryRun, options) {
    ...
    copyTemplateFile('config.json', path.join(targetDir, TASKMANAGER_CONFIG_FILE), {
        ...replacements
    });
    // Copy .gitignore
    ...
    copyTemplateFile('taskmanager.mdc', path.join(targetDir, '.cursor/rules/taskmanager.mdc'));
    ...
    if (!isSilentMode() && !dryRun && !options?.skipSetup) {
        console.log(boxen(chalk.cyan('🔍 Detecting AWS Bedrock Models...'), {
            padding: 0.5,
        ...
        try {
            ...
            if (!options?.yes) {
                ...
                try {
                    ...
                    log('error', 'Failed to configure AI models:', setupError instanceof Error
                        ? setupError.message
                        : String(setupError));
                    log('warn', 'You may need to run "vibex-task-manager models --setup" manually.');
                }
            }
        }
    }
        ...
        log('warn', 'Please configure AI models using "vibex-task-manager models --set-..." or the "models" MCP tool.');
    }
    else if (dryRun) {
        ...
        log('info', 'You can configure models later using "vibex-task-manager config-detect" or "vibex-task-manager models --setup".');
    }
    // ====================================
    // Display success message
    if (!isSilentMode()) {
        console.log(boxen(`${warmGradient.multiline(figlet.textSync('Success!', { font: 'Standard' }))}\n${chalk.green('Project initialized successfully!')}`, {
            padding: 1,
            margin: 1,
            borderStyle: 'double',
            borderColor: 'green'
...
function setupMCPConfiguration(targetDir) {
    ...
    if (fs.existsSync(mcpJsonPath)) {
        log('info', 'MCP configuration file already exists, checking for existing vibex-task-manager-mcp...');
        try {
            ...
            const hasMCPString = Object.values(mcpConfig.mcpServers).some((server) => server.args &&
                server.args.some((arg) => typeof arg === 'string' && arg.includes('vibex-task-manager')));
            if (hasMCPString) {
                log('info', 'Found existing vibex-task-manager MCP configuration in mcp.json, leaving untouched');
            ...
            if (!mcpConfig.mcpServers['vibex-task-manager']) {
                ...
                log('info', 'Added vibex-task-manager server to existing MCP configuration');
            }
            else {
            ...
            log('error', `Failed to update MCP configuration: ${error instanceof Error ? error.message : String(error)}`);
