...
async function analyzeTaskComplexity(options, context = {}) {
    ...
    const thresholdScore = parseFloat(String(options.threshold || '5'));
    ...
    const specificIds = options.id
        ...
        : null;
    const fromId = options.from !== undefined ? parseInt(String(options.from), 10) : null;
    const toId = options.to !== undefined ? parseInt(String(options.to), 10) : null;
    ...
    try {
        ...
        if (options._filteredTasksData) {
            ...
            let filteredTasks = originalData.tasks.filter((task) => activeStatuses.includes(task.status?.toLowerCase() || 'pending'));
            ...
            // Apply range filtering if specified
                ...
                const effectiveToId = toId !== null
                    ? toId
                    : Math.max(...originalData.tasks.map((t) => t.id));
                reportLog(`Filtering tasks by ID range: ${effectiveFromId} to ${effectiveToId}`, 'info');
                filteredTasks = filteredTasks.filter((task) => task.id >= effectiveFromId && task.id <= effectiveToId);
        ...
        if (tasksData.tasks.length === 0) {
            ...
            if (outputFormat === 'text') {
                ...
                console.log(boxen(chalk.white.bold('Suggested Next Steps:') +
                    '\n\n' +
                    `${chalk.cyan('1.')} Run ${chalk.yellow('vibex-task-manager complexity-report')} to review detailed findings\n` +
                    `${chalk.cyan('2.')} Run ${chalk.yellow('vibex-task-manager expand --id=<id>')} to break down complex tasks\n` +
                    `${chalk.cyan('3.')} Run ${chalk.yellow('vibex-task-manager expand --all')} to expand all pending tasks based on complexity`, {
                    padding: 1,
                    borderColor: 'cyan',
                    borderStyle: 'round',
                    margin: { top: 1 }
        ...
        if (outputFormat === 'text') {
            loadingIndicator = startLoadingIndicator(`${useResearch ? 'Researching' : 'Analyzing'} the complexity of your tasks with AI...\n`);
        }
        ...
        try {
            ...
            if (missingTaskIds.length > 0) {
                reportLog(`Missing analysis for ${missingTaskIds.length} tasks: ${missingTaskIds.join(', ')}`, 'warn');
                if (outputFormat === 'text') {
            ...
            if (existingReport && Array.isArray(existingReport.complexityAnalysis)) {
                ...
                const existingEntriesNotAnalyzed = existingReport.complexityAnalysis.filter((item) => !analyzedTaskIds.has(item.taskId));
            ...
            if (outputFormat === 'text') {
                ...
                if (aiError.message.includes('API key')) {
                    ...
                    console.log(chalk.yellow("Run 'vibex-task-manager models --setup' if needed."));
                }
            }
            throw aiError;
        }
...
//# sourceMappingURL=analyze-task-complexity.js.map