...
function parseUpdatedTasksFromText(text, expectedCount, logFn, isMCP) {
    ...
    if (parseMethodUsed === 'raw') {
        ...
        const codeBlockMatch = cleanedResponse.match(/```json\s*([\s\S]*?)\s*```/i // Only match ```json
        );
        if (codeBlockMatch) {
            ...
            if (!prefixFound) {
                report('warn', 'Response does not appear to contain [], JSON code block, or known prefix. Attempting raw parse.');
            }
        }
    }
    ...
    try {
        ...
        report('error', `Extraction method used: ${parseMethodUsed}` // Log which method failed
        );
    ...
    return validationResult.data.slice(0, expectedCount || validationResult.data.length);
}
/**
 ...
 */
async function updateTasks(tasksPath, fromId, prompt, useResearch = false, context = {}, outputFormat = 'text' // Default to text for CLI
) {
    ...
    if (isMCP)
        report('info', `updateTasks called with context: session=${!!session}`);
    else
        report('info', `updateTasks called`); // CLI log
    try {
        if (isMCP)
            report('info', `Updating tasks from ID ${fromId}`);
        else
            report('info', `Updating tasks from ID ${fromId} with prompt: "${prompt}"`);
        ...
        if (outputFormat === 'text') {
            ...
            console.log(boxen(chalk.white.bold(`Updating ${tasksToUpdate.length} tasks`), {
                padding: 1,
                borderColor: 'blue',
                borderStyle: 'round',
...
The changes described in the prompt should be applied to ALL tasks in the list.`;
        ...
        try {
            ...
            const parsedUpdatedTasks = parseUpdatedTasksFromText(aiServiceResponse.mainResult, tasksToUpdate.length, mcpLog || consoleLog, isMCP);
            ...
            if (isMCP)
                report('info', `Successfully updated ${actualUpdateCount} tasks in ${tasksPath}`);
            else
                report('success', `Successfully updated ${actualUpdateCount} tasks in ${tasksPath}`);
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        if (isMCP)
            report('error', `Error updating tasks: ${errorMessage}`);
        else
            report('error', `Error updating tasks: ${errorMessage}`);
        if (outputFormat === 'text') {
