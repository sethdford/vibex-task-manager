...
import { log as consoleLog, readJSON, writeJSON, truncate, isSilentMode } from '../utils.js';
import { getStatusWithColor, startLoadingIndicator, stopLoadingIndicator, displayAiUsageSummary } from '../ui.js';
import { generateTextService } from '../ai-services-unified.js';
import { getDebugFlag, isApiKeySet // Keep this check
...
function parseUpdatedTaskFromText(text, expectedTaskId, logFn, isMCP) {
    ...
    const report = (level, ...args) => {
        if (isMCP) {
            if (typeof logFn[level] === 'function')
                logFn[level](...args);
            else
                logFn.info(...args);
        }
    ...
    if (potentialJsonFromBraces) {
        try {
            ...
            report('info', 'Content between {} looked promising but failed initial parse. Proceeding to other methods.');
    ...
    if (parseMethodUsed === 'raw') {
        ...
        if (codeBlockMatch) {
            ...
            if (!prefixFound) {
                report('warn', 'Response does not appear to contain {}, code block, or known prefix. Attempting raw parse.');
            }
        }
    }
    ...
    try {
        ...
        report('error', `Problematic JSON string (first 500 chars): ${cleanedResponse.substring(0, 500)}`);
        report('error', `Original Raw Response (first 500 chars): ${originalResponseForDebug.substring(0, 500)}`);
    ...
    if (!validationResult.success) {
        ...
        throw new Error(`AI response failed task structure validation: ${validationResult.error.message}`);
    }
    // Final check: ensure ID matches expected ID (AI might hallucinate)
    if (validationResult.data.id !== expectedTaskId) {
        report('warn', `AI returned task with ID ${validationResult.data.id}, but expected ${expectedTaskId}. Overwriting ID.`);
 ...
 */
async function updateTaskById(tasksPath, taskId, prompt, useResearch = false, context = {}, outputFormat = 'text') {
    ...
    try {
        ...
        if (taskToUpdate.status === 'done' || taskToUpdate.status === 'completed') {
            ...
            if (outputFormat === 'text') {
                console.log(boxen(chalk.yellow(`Task ${taskId} is already marked as ${taskToUpdate.status} and cannot be updated.`) +
                    '\n\n' +
                    ...
                    '\n' +
                    chalk.white('2. Then run the update-task command'), { padding: 1, borderColor: 'yellow', borderStyle: 'round' }));
            }
            return null;
        }
        // --- End Task Loading ---
        ...
        if (outputFormat === 'text') {
            ...
            console.log(boxen(chalk.white.bold(`Updating Task #${taskId}`), {
                padding: 1,
                borderColor: 'blue',
                borderStyle: 'round',
            ...
            console.log(boxen(chalk.cyan.bold('How Completed Subtasks Are Handled:') +
                '\n\n' +
...
The changes described in the prompt should be thoughtfully applied to make the task more accurate and actionable.`;
        ...
        if (!isMCP && outputFormat === 'text') {
            loadingIndicator = startLoadingIndicator(useResearch ? 'Updating task with research...\n' : 'Updating task...\n');
        }
        try {
            ...
            const updatedTask = parseUpdatedTaskFromText(aiServiceResponse.mainResult, taskId, mcpLog || consoleLog, isMCP);
            ...
            if (updatedTask.status !== taskToUpdate.status &&
                !prompt.toLowerCase().includes('status')) {
                report('warn', `AI changed task status. Restoring original status '${taskToUpdate.status}'.`);
            ...
            if (taskToUpdate.subtasks?.length > 0) {
                if (!updatedTask.subtasks) {
                    ...
                    const completedOriginal = taskToUpdate.subtasks.filter((st) => st.status === 'done' || st.status === 'completed');
...
//# sourceMappingURL=update-task-by-id.js.map