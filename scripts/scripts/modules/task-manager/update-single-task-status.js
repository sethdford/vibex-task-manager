...
async function updateSingleTaskStatus(tasksPath, taskIdInput, newStatus, data, showUi = true) {
    if (!isValidTaskStatus(newStatus)) {
        throw new Error(`Error: Invalid status value: ${newStatus}. Use one of: ${TASK_STATUS_OPTIONS.join(', ')}`);
    }
    ...
    if (taskIdInput.includes('.')) {
        ...
        if (newStatus.toLowerCase() === 'done' ||
            ...
            const allSubtasksDone = parentTask.subtasks.every((st) => st.status === 'done' || st.status === 'completed');
            ...
            if (allSubtasksDone &&
                ...
                if (showUi) {
                    ...
                    console.log(chalk.yellow(`Consider updating the parent task status with: vibex-task-manager set-status --id=${parentId} --status=done`));
                }
            }
        }
    }
        ...
        if ((newStatus.toLowerCase() === 'done' ||
            ...
            const pendingSubtasks = task.subtasks.filter((st) => st.status !== 'done' && st.status !== 'completed');
...
//# sourceMappingURL=update-single-task-status.js.map