...
import { getStatusWithColor, startLoadingIndicator, stopLoadingIndicator, succeedLoadingIndicator, failLoadingIndicator, displayAiUsageSummary } from '../ui.js';
 ...
 */
async function addTask(tasksPath, prompt, dependencies = [], priority = null, context = {}, outputFormat = 'text', // Default to text for CLI
manualTaskData = null, useResearch = false) {
    ...
    logFn.info(`Adding new task with prompt: "${prompt}", Priority: ${effectivePriority}, Dependencies: ${dependencies.join(', ') || 'None'}, Research: ${useResearch}, ProjectRoot: ${projectRoot}`);
     ...
     * @param visited - Set of already visited task IDs
     * @param depthMap - Map of task ID to its depth in the graph
    ...
    try {
        ...
        if (outputFormat === 'text') {
            console.log(boxen(chalk.white.bold(`Creating New Task #${newTaskId}`), {
                padding: 1,
                borderColor: 'blue',
                borderStyle: 'round',
        ...
        const invalidDeps = dependencies.filter((depId) => {
            ...
            const numDepId = parseInt(String(depId), 10);
            return isNaN(numDepId) || !data.tasks.some((t) => t.id === numDepId);
        });
        if (invalidDeps.length > 0) {
            report(`The following dependencies do not exist or are invalid: ${invalidDeps.join(', ')}`, 'warn');
        ...
        const numericDependencies = dependencies.map((dep) => parseInt(String(dep), 10));
        ...
        if (manualTaskData) {
            ...
            if (outputFormat === 'text') {
                console.log(boxen(chalk.cyan.bold('Task Context Analysis'), {
            ...
            if (numericDependencies.length > 0) {
                ...
                if (dependencyGraphs.length > 0) {
                    ...
                    function formatDependencyChain(node, prefix = '', isLast = true, depth = 0) {
                        if (depth > 3)
                            return ''; // Limit depth to avoid excessive nesting
                        ...
                        if (node.dependencies && node.dependencies.length > 0) {
                            for (let i = 0; i < node.dependencies.length; i++) {
                                ...
                                result += formatDependencyChain(node.dependencies[i], prefix + childPrefix, isLastChild, depth + 1);
                            }
                        }
                        return result;
                    }
                    ...
                    if (dependencyGraphs.length > 0) {
                        ...
                        function visualizeDependencyGraph(node, prefix = '', isLast = true, depth = 0) {
                            ...
                            if (node.dependencies && node.dependencies.length > 0) {
                                for (let i = 0; i < node.dependencies.length; i++) {
                                    const isLastChild = i === node.dependencies.length - 1;
                                    visualizeDependencyGraph(node.dependencies[i], prefix + childPrefix, isLastChild, depth + 1);
                                }
                            }
                        }
                ...
                uniqueDetailedTasks = Array.from(new Map(allDetailedTasks.map((t) => [t.id, t])).values()).slice(0, 20);
                ...
                const relevantPendingTasks = data.tasks
                    ...
                    (relevantTaskIds.has(t.id) ||
                        promptWords.some((word) => t.title.toLowerCase().includes(word) ||
                ...
                const similarPurposeTasks = data.tasks.filter((t) => prompt.toLowerCase().includes(t.title.toLowerCase()));
                ...
                if (outputFormat === 'text') {
                    console.log(chalk.gray(`  Context search across ${data.tasks.length} tasks using full prompt and ${promptWords.length} keywords`));
            ...
            if (outputFormat === 'text') {
                console.log(boxen(chalk.white.bold('AI Task Generation') +
                    ...
                    `\n${chalk.cyan('Detailed tasks: ')}${chalk.yellow(numericDependencies.length > 0
...
`;
      ...
      `;
            ...
            if (outputFormat === 'text') {
                loadingIndicator = startLoadingIndicator(`Generating new task with ${useResearch ? 'Research' : 'Main'} AI... \n`);
            }
            try {
                ...
                else if (aiServiceResponse.mainResult.object &&
        ...
        if (outputFormat === 'text') {
            ...
            console.log(boxen(chalk.white.bold(`Task ${newTaskId} Created Successfully`) +
                '\n\n' +
                ...
                '\n' +
                chalk.cyan(`1. Run ${chalk.yellow(`vibex-task-manager show ${newTaskId}`)} to see complete task details`) +
                '\n' +
                chalk.cyan(`2. Run ${chalk.yellow(`vibex-task-manager set-status --id=${newTaskId} --status=in-progress`)} to start working on it`) +
                '\n' +
                chalk.cyan(`3. Run ${chalk.yellow(`vibex-task-manager expand --id=${newTaskId}`)} to break it down into subtasks`), { padding: 1, borderColor: 'green', borderStyle: 'round' }));
