...
import { z } from 'zod';
import { log, writeJSON, disableSilentMode, isSilentMode, readJSON } from '../utils.js';
...
async function parsePRD(prdPath, tasksPath, numTasks, options = {}) {
    const { reportProgress, mcpLog, session, projectRoot, force = false, append = false, research = false } = options;
    const isMCP = !!mcpLog;
    ...
    try {
        ...
        if (fs.existsSync(tasksPath)) {
            if (append) {
                report(`Append mode enabled. Reading existing tasks from ${tasksPath}`, 'info');
                ...
                if (existingData && Array.isArray(existingData.tasks)) {
                    ...
                    if (existingTasks.length > 0) {
                        ...
                        report(`Found ${existingTasks.length} existing tasks. Next ID will be ${nextId}.`, 'info');
                    }
                }
                else {
                    report(`Could not read existing tasks from ${tasksPath} or format is invalid. Proceeding without appending.`, 'warn');
                    existingTasks = []; // Reset if read fails
                }
            }
            else if (!force) {
                // Not appending and not forcing overwrite
                const overwriteError = new Error(`Output file ${tasksPath} already exists. Use --force to overwrite or --append.`);
                ...
                report(`Force flag enabled. Overwriting existing file: ${tasksPath}`, 'info');
            }
        }
		...
		Return your response in this format:
{
    ...
    "metadata": {
        ...
        if (aiServiceResponse?.mainResult) {
            ...
            if (typeof aiServiceResponse.mainResult === 'object' &&
            ...
            else if (typeof aiServiceResponse.mainResult.object === 'object' &&
        ...
        if (!generatedData || !Array.isArray(generatedData.tasks)) {
            throw new Error('AI service did not return a valid "tasks" array. Please check the AI response.');
        }
        report(`Successfully received and parsed ${generatedData.tasks.length} tasks from AI.`, 'info');
        ...
        const processedNewTasks = generatedData.tasks.map((task, index) => {
            ...
            const validDependencies = (task.dependencies || []).filter((depId) => {
                ...
                if (!existsInOld && !existsInNew) {
                    report(`Task "${task.title}" has an invalid dependency: ID ${depId} does not exist. Removing.`, 'warn');
                    return false;
                }
                return true;
            });
        ...
        if (outputFormat === 'text' && !isSilentMode()) {
            console.log(boxen(chalk.green(`✅ Success! Generated ${processedNewTasks.length} tasks from ${path.basename(prdPath)}.\n\nRun ${chalk.cyan(`vibex-task-manager list`)} to see the new tasks.`), { padding: 1, margin: 1, borderStyle: 'round', borderColor: 'green' }));
        }
