...
function clearSubtasks(tasksPath, taskIds) {
    ...
    if (!isSilentMode()) {
        console.log(boxen(chalk.white.bold('Clearing Subtasks'), {
            padding: 1,
            borderColor: 'blue',
            borderStyle: 'round',
    ...
    if (clearedCount > 0) {
        ...
        // Show summary table
        if (!isSilentMode()) {
            console.log(boxen(chalk.white.bold('Subtask Clearing Summary:'), {
        ...
        // Success message
        if (!isSilentMode()) {
            console.log(boxen(chalk.green(`Successfully cleared subtasks from ${chalk.bold(clearedCount)} task(s)`), {
                padding: 1,
                borderColor: 'green',
                borderStyle: 'round',
                margin: { top: 1 }
            ...
            console.log(boxen(chalk.white.bold('Next Steps:') +
                '\n\n' +
                `${chalk.cyan('1.')} Run ${chalk.yellow('vibex-task-manager expand --id=<id>')} to generate new subtasks\n` +
                `${chalk.cyan('2.')} Run ${chalk.yellow('vibex-task-manager list --with-subtasks')} to verify changes`, {
                padding: 1,
                borderColor: 'cyan',
                borderStyle: 'round',
                margin: { top: 1 }
        ...
        if (!isSilentMode()) {
            console.log(boxen(chalk.yellow('No subtasks were cleared'), {
                padding: 1,
                borderColor: 'yellow',
                borderStyle: 'round',
                margin: { top: 1 }
...
//# sourceMappingURL=clear-subtasks.js.map