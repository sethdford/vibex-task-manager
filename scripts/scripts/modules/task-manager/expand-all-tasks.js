 ...
 */
async function expandAllTasks(tasksPath, numSubtasks, // Keep this signature, expandTask handles defaults
useResearch = false, additionalContext = '', force = false, // Keep force here for the filter logic
context = {}, outputFormat = 'text' // Assume text default for CLI
) {
    ...
    const logger = mcpLog ||
        (outputFormat === 'json'
            ...
            : {
                ...
                debug: (msg) => !isSilentMode() && getDebugFlag(null) && log('debug', msg)
            });
    ...
    try {
        ...
        const tasksToExpand = data.tasks.filter((task) => (task.status === 'pending' || task.status === 'in-progress') && // Include 'in-progress'
        ...
        for (const task of tasksToExpand) {
            ...
            try {
                ...
                const result = await expandTask(tasksPath, task.id, numSubtasks, useResearch, additionalContext, context, // Pass the whole context object { session, mcpLog }
                force);
                expandedCount++;
                ...
                log('error', `Failed to expand task ${task.id}: ${error.message}`);
        ...
        if (outputFormat === 'text') {
            const summaryContent = `${chalk.white.bold('Expansion Summary:')}\n\n` +
        ...
        if (loadingIndicator)
            stopLoadingIndicator(loadingIndicator);
        log('error', `Error during expand all operation: ${error.message}`);
...
//# sourceMappingURL=expand-all-tasks.js.map