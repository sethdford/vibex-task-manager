...
import { startLoadingIndicator, stopLoadingIndicator, displayAiUsageSummary } from '../ui.js';
...
const subtaskSchema = z
    ...
    status: z
        .string()
        .describe('The current status of the subtask (should be pending initially)'),
    testStrategy: z
        .string()
        .optional()
...
function parseSubtasksFromText(text, startId, expectedCount, parentTaskId, logger) {
    ...
    logger.debug(`Original AI Response for parsing (full length: ${jsonToParse.length}): ${jsonToParse.substring(0, 1000)}...`);
    ...
    try {
        ...
        if (!parsedObject ||
            ...
            logger.warn('Simple parse succeeded, but result is not the expected {"subtasks": []} structure. Will proceed to advanced extraction.');
        ...
        logger.warn(`Simple JSON.parse failed. Error: ${e.message}. Will try more robust parsing methods.`);
        primaryParseAttemptFailed = true;
    }
    // --- Attempt 2: Handle cases where the AI returns a raw array instead of an object ---
    if (primaryParseAttemptFailed) {
        logger.debug('Attempting parse assuming a raw array response (wrapping in {"subtasks": ...}).');
        try {
            ...
            if (arrayMatch) {
                ...
                logger.debug('Successfully parsed after wrapping raw array in "subtasks" object.');
            }
            else {
    ...
    if (!parsedObject) {
        ...
        try {
            ...
            logger.debug(`Failed Response Details:
...
Content (first 500 chars): ${jsonToParse.substring(0, 500)}...`);
    ...
    if (!parsedObject || !parsedObject.subtasks) {
        throw new Error('Parsed JSON is invalid or does not contain a "subtasks" array.');
    }
    let subtasks = parsedObject.subtasks;
    // Use Zod to validate the structure of the parsed subtasks
    try {
        subtasks = subtaskArraySchema.parse(subtasks);
    }
    catch (e) {
        logger.error(`Zod validation failed: ${e.message}`);
        logger.debug(`Invalid subtasks object: ${JSON.stringify(subtasks)}`);
        throw new Error(`The AI response, while valid JSON, did not match the required subtask schema. ${e.message}`);
    }
    // --- Post-Parsing Correction and Validation ---
    const correctedSubtasks = [];
    ...
    for (const subtask of subtasks) {
        ...
        if (correctedSubtask.id !== nextId) {
            logger.warn(`Correcting subtask ID from ${correctedSubtask.id} to ${nextId}.`);
            correctedSubtask.id = nextId;
        }
        // 2. Check for duplicate IDs after correction
        if (existingIds.has(correctedSubtask.id)) {
            logger.warn(`Duplicate subtask ID ${correctedSubtask.id} found after correction. Re-assigning.`);
            nextId++; // Get a new ID
            correctedSubtask.id = nextId;
        }
        ...
        correctedSubtask.dependencies = correctedSubtask.dependencies.filter((depId) => {
            if (depId === correctedSubtask.id) {
                logger.warn(`Subtask ${correctedSubtask.id} cannot depend on itself. Removing dependency.`);
                return false;
            }
            if (!validDepIds.has(depId)) {
                logger.warn(`Dependency ID ${depId} for subtask ${correctedSubtask.id} is invalid (out of range). Removing.`);
                return false;
            }
            return true;
        });
    ...
    if (correctedSubtasks.length !== expectedCount) {
        logger.warn(`AI generated ${correctedSubtasks.length} subtasks, but ${expectedCount} were expected. The output will be used as-is, but this may indicate a model compliance issue.`);
    }
 ...
 */
async function expandTask(tasksPath, taskId, numSubtasks, useResearch = false, additionalContext = '', context = {}, force = false) {
    ...
    try {
        if (!isSilent) {
            loadingIndicator = startLoadingIndicator(`Expanding task ${taskId}, please wait...`);
        }
        ...
        const nextSubtaskId = task.subtasks && task.subtasks.length > 0
            ? Math.max(...task.subtasks.map((st) => st.id)) + 1
            : 1;
        report('info', `Expanding task ${taskId} into ${subtaskCount} subtasks. Starting ID: ${nextSubtaskId}. Research mode: ${useResearch}`);
        let aiServiceResponse;
        let generatedSubtasks = [];
        const systemPrompt = generateMainSystemPrompt(subtaskCount);
        const userPrompt = useResearch
            ? generateResearchUserPrompt(task, subtaskCount, additionalContext, nextSubtaskId)
            : generateMainUserPrompt(task, subtaskCount, additionalContext, nextSubtaskId);
        ...
        try {
            ...
            const parsedResult = subtaskWrapperSchema.safeParse(aiServiceResponse.mainResult);
            if (!parsedResult.success) {
                report('error', `AI response failed Zod validation: ${parsedResult.error.message}`);
                ...
                if (typeof aiServiceResponse.mainResult === 'string') {
                    ...
                    generatedSubtasks = parseSubtasksFromText(aiServiceResponse.mainResult, nextSubtaskId, subtaskCount, task.id, mcpLog);
                }
                else {
                    throw new Error('AI response was not a valid JSON object and could not be parsed.');
                }
            }
            else {
                generatedSubtasks = parsedResult.data.subtasks;
            }
        }
        catch (aiError) {
            stopLoadingIndicator(loadingIndicator);
            ...
            if (aiError.response) {
                report('error', `Response details: ${JSON.stringify(aiError.response.data)}`);
            }
        ...
        task.subtasks = generatedSubtasks.map((sub) => ({
            ...sub,
        ...
        try {
            const complexityReportPath = path.join(context.projectRoot || process.cwd(), COMPLEXITY_REPORT_FILE);
            if (fs.existsSync(complexityReportPath)) {
                const complexityReport = readJSON(complexityReportPath);
                const taskComplexity = complexityReport.tasks.find((t) => t.id === task.id);
                if (taskComplexity) {
                    task.complexity = taskComplexity.complexityScore;
                }
            }
        }
        catch (e) {
            report('warn', `Could not read or apply complexity score from report: ${e.message}`);
        }
