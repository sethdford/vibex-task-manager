...
async function setTaskStatus(tasksPath, taskIdInput, newStatus, options = {}) {
    try {
        ...
        if (!isMcpMode) {
            console.log(boxen(chalk.white.bold(`Updating Task Status to: ${newStatus}`), {
                padding: 1,
                borderColor: 'blue',
                borderStyle: 'round'
            }));
        ...
        if (!isMcpMode) {
            for (const id of updatedTasks) {
                ...
                console.log(boxen(chalk.white.bold(`Successfully updated task ${id} status:`) +
                    ...
                    `To:   ${chalk.green(newStatus)}`, { padding: 1, borderColor: 'green', borderStyle: 'round' }));
            }
        }
...
//# sourceMappingURL=set-task-status.js.map