...
import { log, readJSON, writeJSON, formatTaskId, findCycles, isSilentMode } from './utils.js';
...
async function addDependency(tasksPath, taskId, dependencyId) {
    ...
    if (typeof formattedTaskId === 'string' &&
    ...
    if (!isCircularDependency(data.tasks, formattedDependencyId, dependencyChain)) {
        ...
        if (!isSilentMode()) {
            console.log(boxen(chalk.green(`Successfully added dependency:\n\n`) +
...
async function removeDependency(tasksPath, taskId, dependencyId) {
    ...
    data.tasks.forEach((task) => {
        if (String(task.id) === String(formattedTaskId)) {
            ...
            if (task.dependencies) {
                ...
                task.dependencies = task.dependencies.filter((d) => String(d) !== String(formattedDependencyId));
            ...
            task.subtasks.forEach((subtask) => {
                if (String(subtask.id) === String(formattedTaskId)) {
                    ...
                    if (subtask.dependencies) {
                        ...
                        subtask.dependencies = subtask.dependencies.filter((d) => String(d) !== String(formattedDependencyId));
    ...
    if (!dependencyFound) {
        log('warn', `Dependency ${formattedDependencyId} not found in task ${formattedTaskId}.`);
        return;
    }
    // Save changes and regenerate files
    writeJSON(tasksPath, data);
    log('success', `Removed dependency ${formattedDependencyId} from task ${formattedTaskId}.`);
    if (!isSilentMode()) {
        console.log(boxen(chalk.green(`Successfully removed dependency ${chalk.bold(formattedDependencyId)} from task ${chalk.bold(formattedTaskId)}.`), { padding: 1, borderColor: 'green', margin: { top: 1 } }));
    }
    await generateTaskFiles(tasksPath, path.dirname(tasksPath));
...
function isCircularDependency(tasks, taskId, chain = []) {
    ...
    if (task) {
        ...
        for (const parentTask of tasks) {
            if (parentTask.subtasks) {
                const subtask = parentTask.subtasks.find((s) => formatTaskId(String(s.id)) === currentTaskId);
                if (subtask) {
    ...
    for (const depId of dependencies) {
        ...
        if (newChain.includes(formattedDepId)) {
            log('warn', `Circular dependency detected: ${newChain.join(' -> ')} -> ${formattedDepId}`);
            return true;
        }
...
function cleanupSubtaskDependencies(tasksData) {
    ...
    tasksData.tasks.forEach((task) => {
        if (task.dependencies && task.subtasks) {
            ...
            task.dependencies = task.dependencies.filter((depId) => !subtaskIds.has(depId));
        }
    });
    return tasksData;
}
...
async function validateDependenciesCommand(tasksPath, options = {}) {
    ...
    if (validationResult.valid) {
        ...
        if (!isSilentMode()) {
            console.log(boxen(chalk.green('✅ All dependencies are valid.'), {
                padding: 1,
                borderColor: 'green',
                margin: 1
            }));
        ...
        if (!isSilentMode()) {
            console.log(boxen([
                ...
                ...errorMessages,
                '',
                chalk.cyan('Run `vibex-task-manager fix-dependencies` to attempt to resolve these issues.')
...
async function fixDependenciesCommand(tasksPath, options = {}) {
    ...
    if (issues.length === 0) {
        ...
        if (!isSilentMode()) {
            console.log(boxen(chalk.green('✅ No issues found.'), {
                padding: 1,
                borderColor: 'green',
                margin: 1
            }));
        ...
        missingDeps.forEach((issue) => {
            data.tasks.forEach((task) => {
                if (String(task.id) === String(issue.taskId)) {
                    task.dependencies = task.dependencies.filter((depId) => String(depId) !== String(issue.dependencyId));
                    changesMade = true;
                    log('info', `Removed missing dependency '${issue.dependencyId}' from task ${issue.taskId}`);
                }
                if (task.subtasks) {
                    task.subtasks.forEach((subtask) => {
                        if (String(subtask.id) === String(issue.taskId)) {
                            subtask.dependencies = subtask.dependencies.filter((depId) => String(depId) !== String(issue.dependencyId));
                            changesMade = true;
                            log('info', `Removed missing dependency '${issue.dependencyId}' from subtask ${issue.taskId}`);
                        }
                    });
                }
            });
                ...
                if (task.subtasks) {
                    task.subtasks.forEach((subtask) => {
                        if (String(subtask.id) === String(issue.taskId)) {
                            subtask.dependencies = subtask.dependencies.filter((depId) => String(depId) !== String(issue.dependencyId));
                            changesMade = true;
                            log('info', `Removed self-dependency from subtask ${issue.taskId}`);
                        }
                    });
                }
            });
    ...
    if (circularDeps.length > 0) {
        log('warn', 'Circular dependencies detected. These must be resolved manually.');
    ...
    if (changesMade) {
        ...
        if (!isSilentMode()) {
            console.log(boxen(chalk.green('✅ Dependency issues have been fixed.'), {
                padding: 1,
                borderColor: 'green',
                margin: 1
            }));
 ...
 */
function validateSubtaskDependencies(parentTask) {
    const issues = [];
    ...
    const subtaskGraph = new Map();
    if (parentTask.subtasks) {
        for (const subtask of parentTask.subtasks) {
            const subtaskId = `${parentTask.id}.${subtask.id}`;
            const dependencies = subtask.dependencies?.map((d) => typeof d === 'number' ? d.toString() : d) || [];
    ...
    for (const subtask of parentTask.subtasks || []) {
        ...
        if (cycle.length > 0) {
            issues.push({
                type: 'circular',
                taskId: subtaskId,
                message: `Circular dependency detected in subtasks of task ${parentTask.id}: ${cycle.join(' -> ')}`
            });
        }
    }
    return issues;
...
function ensureAtLeastOneIndependentSubtask(tasksData) {
    ...
    tasksData.tasks.forEach((task) => {
        if (task.subtasks && task.subtasks.length > 0) {
            const hasIndependentSubtask = task.subtasks.some((s) => !s.dependencies || s.dependencies.length === 0);
            if (!hasIndependentSubtask) {
                ...
                log('warn', `No independent subtask found for parent task ${task.id}. Making the first subtask independent.`);
