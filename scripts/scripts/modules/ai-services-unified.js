 ...
 */
// --- Core Dependencies ---
import { getMainProvider, getMainModelId, getResearchProvider, getResearchModelId, getFallbackProvider, getFallbackModelId, getUserId, MODEL_MAP, getDebugFlag, getBaseUrlForRole, isApiKeySet } from './config-manager.js';
...
function _getCostForModel(providerName, modelId) {
    ...
    const providerMap = MODEL_MAP[providerName];
    if (!providerMap) {
        log('warn', `Provider "${providerName}" not found. Cannot determine cost.`);
        return { inputCost: 0, outputCost: 0, currency: 'USD' };
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const modelData = providerMap.find((m) => m.id === modelId);
...
function _resolveApiKey(providerName, session, projectRoot = null) {
    ...
    if (!apiKey) {
        throw new Error(`Required API key ${envVarName} for provider '${providerName}' is not set in environment, session, or .env file.`);
    }
    return apiKey;
}
/**
 ...
 */
async function _attemptProviderCallWithRetries(provider, serviceType, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
callParams, providerName, modelId, attemptRole) {
    let retries = 0;
    const fnName = serviceType;
    while (retries <= MAX_RETRIES) {
        try {
            if (getDebugFlag()) {
                log('info', `Attempt ${retries + 1}/${MAX_RETRIES + 1} calling ${fnName} (Provider: ${providerName}, Model: ${modelId}, Role: ${attemptRole})`);
            }
            // Call the appropriate method on the provider instance
            const result = await provider[serviceType](callParams);
            if (getDebugFlag()) {
                log('info', `${fnName} succeeded for role ${attemptRole} (Provider: ${providerName}) on attempt ${retries + 1}`);
            }
            return result;
        }
        catch (error) {
            const message = error instanceof Error ? error.message : 'An unknown error occurred';
            log('warn', `Attempt ${retries + 1} failed for role ${attemptRole} (${fnName} / ${providerName}): ${message}`);
            if (isRetryableError(error) && retries < MAX_RETRIES) {
                ...
                log('error', `Something went wrong on the provider side. Max retries reached for role ${attemptRole} (${fnName} / ${providerName}).`);
...
async function _unifiedServiceRunner(serviceType, params) {
    const { role: initialRole, session, projectRoot, systemPrompt, prompt, schema, objectName, commandName, outputType, ...restApiParams } = params;
    if (getDebugFlag()) {
    ...
    const attemptSequence = [
        {
            role: initialRole,
            getProvider: initialRole === 'research' ? getResearchProvider : getMainProvider,
            getModelId: initialRole === 'research' ? getResearchModelId : getMainModelId,
    ...
    if (fallbackProvider &&
        ...
        attemptSequence.push({
            role: 'fallback',
            getProvider: (explicitRoot) => getFallbackProvider(explicitRoot) || '',
            getModelId: (explicitRoot) => getFallbackModelId(explicitRoot) || '',
            getBaseUrl: getBaseUrlForRole
        });
    }
    for (const attempt of attemptSequence) {
        ...
        if (!providerName || !modelId) {
            log('debug', `Skipping attempt for role '${attempt.role}' because provider or model is not configured.`);
            continue;
        }
        // For Bedrock, we don't check API keys but rely on AWS credential resolution
        if (providerName?.toLowerCase() !== 'bedrock') {
            if (!isApiKeySet(providerName, session, effectiveProjectRoot)) {
                log('warn', `Skipping attempt for role '${attempt.role}' (${providerName}) due to missing API key.`);
                continue;
            }
        }
        else {
        ...
        try {
            ...
            const result = await _attemptProviderCallWithRetries(provider, serviceType, callParams, providerName, modelId, attempt.role);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const { text: mainResult = null, usage = {} } = (result || {});
            ...
            if (getDebugFlag()) {
                log('info', `Successfully executed ${serviceType} with role '${attempt.role}'`, {
                    result,
            ...
            return {
                mainResult,
                telemetryData: telemetryPayloads.length > 0
                    ? telemetryPayloads[telemetryPayloads.length - 1]
                    : null
            };
        }
        catch (error) {
            log('error', `Service call failed for role '${attempt.role}' with provider '${providerName}'.`, error instanceof Error ? error.message : String(error));
            // Don't re-throw here; loop to the next provider (fallback)
        }
    }
    log('error', `All configured AI providers (${attemptSequence.map((a) => a.role).join(', ')}) failed for ${serviceType}.`);
    return null;
}
/**
...
async function logAiUsage({ userId, commandName, providerName, modelId, inputTokens, outputTokens, outputType }) {
    if (outputType !== 'cli')
        return null; // Only log for CLI usage for now
    if (!userId) {
    ...
    try {
        ...
        const telemetryData = {
            ...
            totalCost: (inputTokens / 1_000_000) * inputCost +
...
//# sourceMappingURL=ai-services-unified.js.map