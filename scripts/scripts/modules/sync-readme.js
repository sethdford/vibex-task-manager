...
This project is managed using Vibex Task Manager.

`;
}
/**
 * Create UTM tracking URL for vibex-task-manager.dev
 * @param {string} projectRoot - The project root path
 * @returns {string} - UTM tracked URL
 */
function createVibexTaskManagerUrl(projectRoot) {
    ...
    return `https://vibex-task-manager.dev?${utmParams.toString()}`;
}
/**
...
function createStartMarker(options) {
    ...
    const exportInfo = `🎯 **Vibex Task Manager Export** - ${timestamp}\n` +
        `📋 Export: ${subtasksText} • ${statusText}\n` +
        `🔗 Powered by [Vibex Task Manager](${createVibexTaskManagerUrl(projectRoot)})`;
    // Create a markdown box using code blocks and emojis to mimic our UI style
    const boxContent = `<!-- TASKMANAGER_EXPORT_START -->\n` +
        `> ${exportInfo.split('\n').join('\n> ')}\n\n`;
    return boxContent;
}
/**
...
export async function syncTasksToReadme(projectRoot = null, options = {}) {
    try {
        ...
        const tasksOutput = await listTasks(tasksPath ||
            path.join(actualProjectRoot, '.taskmaster', 'tasks', 'tasks.json'), status || 'pending', null, withSubtasks, 'markdown-readme');
        if (!tasksOutput) {
        ...
        const startComment = '<!-- TASKMANAGER_EXPORT_START -->';
        const endComment = '<!-- TASKMANAGER_EXPORT_END -->';
        let updatedContent;
        ...
        console.log(chalk.cyan(`📋 Export details: ${withSubtasks ? 'with' : 'without'} subtasks${status ? `, status: ${status}` : ''}`));
...
//# sourceMappingURL=sync-readme.js.map