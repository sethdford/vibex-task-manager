...
import { log, findTaskById, readJSON, truncate, isSilentMode } from './utils.js';
import fs from 'fs';
import { findNextTask, analyzeTaskComplexity, readComplexityReport } from './task-manager.js';
import { getProjectName, getDefaultSubtasks } from './config-manager.js';
import { TASK_STATUS_OPTIONS } from '../../src/constants/task-status.js';
import { TASKMANAGER_CONFIG_FILE, TASKMANAGER_TASKS_FILE } from '../../src/constants/paths.js';
...
function displayBanner() {
    ...
    const bannerText = figlet.textSync('Vibex TM', {
        font: 'Standard',
    ...
    console.log(boxen(chalk.white(`${chalk.bold('Version:')} ${version}   ${chalk.bold('Project:')} ${getProjectName(null)}`), {
        padding: 1,
 ...
 */
function formatDependenciesWithStatus(dependencies, allTasks, forConsole = false, complexityReport = null // Add complexityReport parameter
) {
    if (!dependencies ||
...
function displayHelp() {
    ...
    console.log(boxen(chalk.white.bold('Task Manager CLI'), {
        padding: 1,
        borderColor: 'blue',
        borderStyle: 'round',
    ...
    commandCategories.forEach((category) => {
        console.log(boxen(chalk[category.color].bold(category.title), {
    ...
    console.log(boxen(chalk.cyan.bold('Configuration'), {
        padding: { left: 2, right: 2, top: 0, bottom: 0 },
        margin: { top: 1, bottom: 0 },
        borderColor: 'cyan',
        borderStyle: 'round'
    ...
    console.log('');
    // Show helpful hints
    console.log(boxen(chalk.white.bold('Quick Start:') +
        '\n\n' +
...
async function displayNextTask(tasksPath, complexityReportPath = null) {
    ...
    if (!nextTask) {
        console.log(boxen(chalk.yellow('No eligible tasks found!\n\n') +
    ...
    taskTable.push([chalk.cyan.bold('ID:'), nextTask.id.toString()], [chalk.cyan.bold('Title:'), nextTask.title], [
    ...
    if (nextTask.details && nextTask.details.trim().length > 0) {
        console.log(boxen(chalk.white.bold('Implementation Details:') + '\n\n' + nextTask.details, {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            borderColor: 'cyan',
            borderStyle: 'round',
            margin: { top: 1, bottom: 0 }
    ...
    if (!isSubtask && nextTask.subtasks && nextTask.subtasks.length > 0) {
        console.log(boxen(chalk.white.bold('Subtasks'), {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            margin: { top: 1, bottom: 0 },
            borderColor: 'magenta',
            borderStyle: 'round'
            ...
            if (st.dependencies && st.dependencies.length > 0) {
                ...
                const formattedDeps = st.dependencies.map((depId) => {
                    if (typeof depId === 'number' && depId < 100) {
                        ...
                        if (foundSubtask) {
                            const isDone = foundSubtask.status === 'done' ||
    ...
    if (!isSubtask && (!nextTask.subtasks || nextTask.subtasks.length === 0)) {
        console.log(boxen(chalk.yellow('No subtasks found. Consider breaking down this task:') +
            '\n' +
            chalk.white(`Run: ${chalk.cyan(`vibex-task-manager expand --id=${nextTask.id}`)}`), {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            borderColor: 'yellow',
            borderStyle: 'round',
            margin: { top: 1, bottom: 0 }
    ...
    if (isSubtask) {
        ...
        suggestedActionsContent +=
            `${chalk.cyan('1.')} Mark as in-progress: ${chalk.yellow(`vibex-task-manager set-status --id=${nextTask.id} --status=in-progress`)}\n` +
                ...
                `${chalk.cyan('3.')} View parent task: ${chalk.yellow(`vibex-task-manager show --id=${nextTask.parentId}`)}`;
    }
    else {
        ...
        suggestedActionsContent +=
            `${chalk.cyan('1.')} Mark as in-progress: ${chalk.yellow(`vibex-task-manager set-status --id=${nextTask.id} --status=in-progress`)}\n` +
                ...
                (nextTask.subtasks && nextTask.subtasks.length > 0
                    ? `${chalk.cyan('3.')} Update subtask status: ${chalk.yellow(`vibex-task-manager set-status --id=${nextTask.id}.1 --status=done`)}` // Example: first subtask
 ...
 */
async function displayTaskById(tasksPath, taskId, complexityReportPath = null, statusFilter = null) {
    // Read the tasks file
    const data = readJSON(tasksPath);
    if (!data || !data.tasks) {
        log('error', 'No valid tasks found.');
    ...
    const taskResult = findTaskById(data.tasks, taskId, complexityReport, statusFilter);
    const task = taskResult.task;
    const originalSubtaskCount = taskResult.originalSubtaskCount;
    const originalSubtasks = taskResult.originalSubtasks;
    if (!task) {
        console.log(boxen(chalk.yellow(`Task with ID ${taskId} not found!`), {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            borderColor: 'yellow',
            borderStyle: 'round',
            margin: { top: 1 }
    ...
    if (task.isSubtask || task.parentTask) {
        console.log(boxen(chalk.white.bold(`Subtask: #${task.parentTask?.id}.${task.id} - ${task.title}`), {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            borderColor: 'magenta',
            borderStyle: 'round',
            margin: { top: 1, bottom: 0 }
        ...
        subtaskTable.push([chalk.cyan.bold('ID:'), `${task.parentTask.id}.${task.id}`], [
        ...
        console.log(boxen(chalk.white.bold('Suggested Actions:') +
            '\n' +
            `${chalk.cyan('1.')} Mark as in-progress: ${chalk.yellow(`vibex-task-manager set-status --id=${task.parentTask.id}.${task.id} --status=in-progress`)}\n` +
            `${chalk.cyan('2.')} Mark as done when completed: ${chalk.yellow(`vibex-task-manager set-status --id=${task.parentTask.id}.${task.id} --status=done`)}\n` +
            `${chalk.cyan('3.')} View parent task: ${chalk.yellow(`vibex-task-manager show --id=${task.parentTask.id}`)}`, {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            borderColor: 'green',
            borderStyle: 'round',
            margin: { top: 1 }
    ...
    console.log(boxen(chalk.white.bold(`Task: #${task.id} - ${task.title}`), {
        padding: { top: 0, bottom: 0, left: 1, right: 1 },
        borderColor: 'blue',
        borderStyle: 'round',
        margin: { top: 1, bottom: 0 }
    ...
    taskTable.push([chalk.cyan.bold('ID:'), task.id.toString()], [chalk.cyan.bold('Title:'), task.title], [
        chalk.cyan.bold('Status:'),
        getStatusWithColor(task.status || 'pending', true)
    ...
    if (task.testStrategy && task.testStrategy.trim().length > 0) {
        console.log(boxen(chalk.white.bold('Test Strategy:') + '\n\n' + task.testStrategy, {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            borderColor: 'cyan',
            borderStyle: 'round',
            margin: { top: 1, bottom: 0 }
    ...
    if (task.subtasks && task.subtasks.length > 0) {
        ...
        task.subtasks.forEach((st) => {
            ...
            if (st.dependencies && st.dependencies.length > 0) {
                const formattedDeps = st.dependencies.map((depId) => {
                    ...
                    const foundDepSubtask = typeof depId === 'number' && depId < 100
                        ? sourceListForDeps.find((sub) => sub.id === depId)
                        : null;
                    if (foundDepSubtask) {
                        const isDone = foundDepSubtask.status === 'done' ||
        ...
        if (statusFilter && originalSubtaskCount !== null) {
            console.log(chalk.cyan(`  Filtered by status: ${chalk.bold(statusFilter)}. Showing ${chalk.bold(task.subtasks.length)} of ${chalk.bold(originalSubtaskCount)} subtasks.`));
        ...
        console.log(boxen(chalk.yellow(`No subtasks found matching status: ${statusFilter} (Task has no subtasks)`), {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            margin: { top: 1, bottom: 0 },
            borderColor: 'yellow',
            borderStyle: 'round'
        ...
        console.log(boxen(chalk.yellow(`No subtasks found matching status: ${statusFilter} (out of ${originalSubtaskCount} total)`), {
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            margin: { top: 1, bottom: 0 },
            borderColor: 'yellow',
            borderStyle: 'round'
        ...
        if (!actualSubtasks || actualSubtasks.length === 0) {
            console.log(boxen(chalk.yellow('No subtasks found. Consider breaking down this task:') +
                '\n' +
                chalk.white(`Run: ${chalk.cyan(`vibex-task-manager expand --id=${task.id}`)}`), {
                padding: { top: 0, bottom: 0, left: 1, right: 1 },
                borderColor: 'yellow',
                borderStyle: 'round',
                margin: { top: 1, bottom: 0 }
    ...
    if (subtasksForProgress && subtasksForProgress.length > 0) {
        ...
        const completedSubtasks = subtasksForProgress.filter((st) => st.status === 'done' || st.status === 'completed').length;
        // Count other statuses from the original/complete list
        const inProgressSubtasks = subtasksForProgress.filter((st) => st.status === 'in-progress').length;
        ...
        const progressBarLength = Math.max(20, Math.min(60, availableWidth - boxPadding - boxBorders - percentTextLength - 35));
        const statusCounts = `${chalk.green('✓ Done:')} ${completedSubtasks}  ${chalk.hex('#FFA500')('► In Progress:')} ${inProgressSubtasks}  ${chalk.yellow('○ Pending:')} ${pendingSubtasks}\n` +
            `${chalk.red('! Blocked:')} ${blockedSubtasks}  ${chalk.gray('⏱ Deferred:')} ${deferredSubtasks}  ${chalk.gray('✗ Cancelled:')} ${cancelledSubtasks}`;
        console.log(boxen(chalk.white.bold('Subtask Progress:') +
            '\n\n' +
    ...
    console.log(boxen(chalk.white.bold('Suggested Actions:') +
        ...
        (subtasksForProgress && subtasksForProgress.length > 0
            ? `${chalk.cyan('3.')} Update subtask status: ${chalk.yellow(`vibex-task-manager set-status --id=${task.id}.1 --status=done`)}` // Example uses .1
...
async function displayComplexityReport(reportPath) {
    ...
    if (!fs.existsSync(reportPath)) {
        console.log(boxen(chalk.yellow(`No complexity report found at ${reportPath}\n\n`) +
        ...
        if (String(answer).toLowerCase() === 'y' ||
            ...
            if (!fs.existsSync(tasksPath)) {
                console.error('❌ No tasks.json file found. Please run "vibex-task-manager init" or create a tasks.json file.');
                return null;
            }
    ...
    console.log(boxen(chalk.white.bold('Task Complexity Analysis Report'), {
        padding: 1,
        borderColor: 'blue',
        borderStyle: 'round',
        margin: { top: 1, bottom: 1 }
    ...
    ], [chalk.cyan.bold('Tasks Analyzed:'), report.meta.tasksAnalyzed], [chalk.cyan.bold('Threshold Score:'), report.meta.thresholdScore], [chalk.cyan.bold('Project:'), report.meta.projectName], [
    ...
    console.log(boxen(chalk.white.bold('Complexity Distribution\n\n') +
    ...
    if (simpleTasks.length > 0) {
        console.log(boxen(chalk.green.bold(`Simple Tasks (${simpleTasks.length})`), {
            padding: { left: 2, right: 2, top: 0, bottom: 0 },
            margin: { top: 1, bottom: 0 },
            borderColor: 'green',
            borderStyle: 'round'
...
async function confirmTaskOverwrite(tasksPath) {
    console.log(boxen(chalk.yellow("It looks like you've already generated tasks for this project.\n") +
...
function displayApiKeyStatus(statusReport) {
    ...
    console.log(chalk.gray(`  Note: Some providers (e.g., Azure, Ollama) may require additional endpoint configuration in ${TASKMANAGER_CONFIG_FILE}.`));
}
...
function displayAvailableModels(availableModels) {
    ...
    console.log(boxen(chalk.white.bold('Next Steps:') +
        '\n' +
        chalk.cyan(`1. Set main model: ${chalk.yellow('vibex-task-manager models --set-main <model_id>')}`) +
        '\n' +
        chalk.cyan(`2. Set research model: ${chalk.yellow('vibex-task-manager models --set-research <model_id>')}`) +
        '\n' +
        chalk.cyan(`3. Set fallback model: ${chalk.yellow('vibex-task-manager models --set-fallback <model_id>')}`) +
        '\n' +
        chalk.cyan(`4. Run interactive setup: ${chalk.yellow('vibex-task-manager models --setup')}`) +
        '\n' +
        chalk.cyan(`5. Use custom ollama/openrouter models: ${chalk.yellow('vibex-task-manager models --openrouter|ollama --set-main|research|fallback <model_id>')}`), {
        padding: 1,
        borderColor: 'yellow',
        borderStyle: 'round',
        margin: { top: 1 }
...
function displayAiUsageSummary(telemetryData, outputType = 'cli') {
    if ((outputType !== 'cli' && outputType !== 'text') ||
        !telemetryData ||
        isSilentMode()) {
        return; // Only display for CLI and if data exists and not in silent mode
    }
    const { modelUsed, providerName, inputTokens, outputTokens, totalTokens, totalCost, commandName } = telemetryData;
