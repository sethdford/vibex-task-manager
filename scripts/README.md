...
- AWS Bedrock model access enabled for Claude and Amazon Titan models

### Optional Configuration

- `AWS_PROFILE`: AWS profile to use (default: uses AWS SDK credential chain)
- `AWS_DEFAULT_REGION`: AWS region for Bedrock (default: us-east-1)
- `BEDROCK_BASE_URL`: Custom Bedrock endpoint URL
- `DEBUG`: Enable debug logging (default: false)
- `VIBEX_LOG_LEVEL`: Log level - debug, info, warn, error (default: info)
- `DEFAULT_SUBTASKS`: Default number of subtasks when expanding (default: 5)
