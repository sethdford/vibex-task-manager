...
// --- Core Dependencies ---
import { getMainProvider, getMainModelId, getResearchProvider, getResearchModelId, getFallbackProvider, getFallbackModelId, getParametersForRole, getUserId, MODEL_MAP, getDebugFlag, getBaseUrlForRole, isApiKeySet, getBedrockBaseURL } from './config-manager.js';
...
function _getCostForModel(providerName, modelId) {
    if (!MODEL_MAP || !MODEL_MAP[providerName]) {
        log('warn', `Provider "${providerName}" not found in MODEL_MAP. Cannot determine cost for model ${modelId}.`);
    ...
    if (!modelData || !modelData.cost_per_1m_tokens) {
        log('debug', `Cost data not found for model "${modelId}" under provider "${providerName}". Assuming zero cost.`);
        return { inputCost: 0, outputCost: 0, currency: 'USD' }; // Default to zero cost
    }
...
async function _attemptProviderCallWithRetries(provider, serviceType, callParams, providerName, modelId, attemptRole) {
    ...
    while (retries <= MAX_RETRIES) {
        try {
            ...
            log('warn', `Attempt ${retries + 1} failed for role ${attemptRole} (${fnName} / ${providerName}): ${error.message}`);
...
async function _unifiedServiceRunner(serviceType, params) {
    ...
    if (initialRole === 'main') {
        ...
        log('warn', `Unknown initial role: ${initialRole}. Defaulting to main -> fallback -> research sequence.`);
        sequence = ['main', 'fallback', 'research'];
    }
    let lastError = null;
    ...
    for (const currentRole of sequence) {
        ...
        try {
            ...
            if (!providerName || !modelId) {
                ...
                lastError =
                    lastError ||
                        new Error(`Configuration missing for role '${currentRole}'. Provider: ${providerName}, Model: ${modelId}`);
                continue;
            }
            ...
            if (providerName?.toLowerCase() !== 'bedrock') {
                if (!isApiKeySet(providerName, session, effectiveProjectRoot)) {
                    log('warn', `Skipping role '${currentRole}' (Provider: ${providerName}): API key not set or invalid.`);
                    lastError =
                        lastError ||
            ...
            if (userId && providerResponse && providerResponse.usage) {
                ...
                log('warn', `Cannot log telemetry for ${commandName} (${providerName}/${modelId}): AI result missing 'usage' data. (May be expected for streams)`);
            }
            let finalMainResult;
            ...
            if (serviceType === 'generateObject') {
                ...
                if (lowerCaseMessage.includes('no endpoints found that support tool use') ||
...
async function logAiUsage({ userId, commandName, providerName, modelId, inputTokens, outputTokens, outputType }) {
    ...
    try {
        ...
        const totalCost = ((inputTokens || 0) / 1_000_000) * inputCost +
...
export { generateTextService, streamTextService, generateObjectService, logAiUsage };
