    ...
    try {
        ...
        if (providerModels && Array.isArray(providerModels)) {
            ...
            if (modelDefinition &&
                ...
                log('debug', `Applying model-specific max_tokens (${modelSpecificMaxTokens}) for ${modelId}. Effective limit: ${effectiveMaxTokens}`);
            }
            else {
                log('debug', `No valid model-specific max_tokens override found for ${modelId}. Using role default: ${roleMaxTokens}`);
            }
        }
        else {
            log('debug', `No model definitions found for provider ${providerName} in MODEL_MAP. Using role default maxTokens: ${roleMaxTokens}`);
        }
    }
    catch (lookupError) {
        log('warn', `Error looking up model-specific max_tokens for ${modelId}: ${lookupError.message}. Using role default: ${roleMaxTokens}`);
...
function isApiKeySet(providerName, session = null, projectRoot = null) {
    ...
    return (apiKeyValue &&
        ...
        !apiKeyValue.includes('KEY_HERE')); // Another common placeholder pattern
}
/**
    ...
    for (const [provider, models] of Object.entries(MODEL_MAP)) {
        if (models.length > 0) {
            models.forEach((modelObj) => {
                ...
                if (modelId === 'sonar-mini')
                    name = 'Perplexity Sonar Mini';
                available.push({
                    id: modelId,
                    name: name,
                    provider: provider,
...
function getUserId(explicitRoot = null) {
    ...
    if (!config.global.userId) {
        ...
        if (!success) {
            ...
            log('warn', 'Failed to write updated configuration with new userId. Please let the developers know.');
        }
    }
