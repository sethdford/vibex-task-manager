/**
 * commands.js
 * Command-line interface for the Task Manager CLI
 */
...
import { parsePRD, updateTasks, generateTaskFiles, setTaskStatus, listTasks, expandTask, expandAllTasks, clearSubtasks, addTask, addSubtask, removeSubtask, analyzeTaskComplexity, updateTaskById, updateSubtaskById, removeTask, findTaskById, taskExists, moveTask, migrateProject } from './task-manager.js';
import { addDependency, removeDependency, validateDependenciesCommand, fixDependenciesCommand } from './dependency-manager.js';
import { isApiKeySet, getDebugFlag, getConfig, writeConfig, ConfigurationError, isConfigFilePresent, getAvailableModels } from './config-manager.js';
import { COMPLEXITY_REPORT_FILE, PRD_FILE, TASKMANAGER_TASKS_FILE } from '../../src/constants/paths.js';
import { displayBanner, displayHelp, displayNextTask, displayTaskById, displayComplexityReport, getStatusWithColor, confirmTaskOverwrite, startLoadingIndicator, stopLoadingIndicator, displayModelConfiguration, displayAvailableModels, displayApiKeyStatus } from './ui.js';
import { initializeProject } from '../init.js';
import { getModelConfiguration, getAvailableModelsList, setModel, getApiKeyStatusReport } from './task-manager/models.js';
...
async function runInteractiveSetup(projectRoot) {
    ...
    if (!currentConfigResult.success &&
        ...
        console.warn(chalk.yellow(`Warning: Could not load current model configuration: ${currentConfigResult.error?.message || 'Unknown error'}. Proceeding with defaults.`));
    }
    ...
    const getPromptData = (role, allowNone = false) => {
        ...
        const roleChoices = Object.entries(modelsByProvider)
            ...
            const providerModels = models
                ...
                name: `${provider} / ${m.id} ${m.cost_per_1m_tokens
                    ? chalk.gray(`($${m.cost_per_1m_tokens.input.toFixed(2)} input | $${m.cost_per_1m_tokens.output.toFixed(2)} output)`)
                    : ''}`,
        ...
        if (currentModel?.modelId && currentModel?.provider) {
            currentChoiceIndex = roleChoices.findIndex((choice) => typeof choice?.value === 'object' &&
        ...
        if (defaultIndex < 0 || defaultIndex >= choices.length) {
            ...
            console.warn(`Warning: Could not determine default model for role '${role}'. Defaulting to 'Cancel'.`); // Add warning
        }
    ...
    const answers = await inquirer.prompt([
        ...
        },
        {
            ...
            when: (ans) => ans.mainModel !== '__CANCEL__' && ans.researchModel !== '__CANCEL__'
        }
    ]);
    ...
    async function handleSetModel(role, selectedValue, currentModelId) {
        ...
        // let isCustomSelection = false; // Unused variable
        if (selectedValue === '__CUSTOM_OPENROUTER__') {
            // isCustomSelection = true; // Unused variable
            ...
            // Note: OpenRouter validation removed - models are now validated at runtime
            console.log(chalk.blue(`Custom OpenRouter model "${modelIdToSet}" will be used. Validation will occur at runtime.`));
        }
            ...
            modelIdToSet = customId;
            providerHint = 'ollama';
            // Note: Ollama validation removed - models are now validated at runtime
            console.log(chalk.blue(`Custom Ollama model "${modelIdToSet}" will be used. Validation will occur at runtime.`));
        }
            ...
            if (!process.env.AWS_ACCESS_KEY_ID ||
                ...
                console.error(chalk.red('Error: AWS_ACCESS_KEY_ID and/or AWS_SECRET_ACCESS_KEY environment variables are missing. Please set them before using custom Bedrock models.'));
                setupSuccess = false;
                return true; // Continue setup, but mark as failed
            }
            console.log(chalk.blue(`Custom Bedrock model "${modelIdToSet}" will be used. No validation performed.`));
        }
            ...
            console.error(chalk.red(`Internal Error: Unexpected selection value for ${role}: ${JSON.stringify(selectedValue)}`));
            setupSuccess = false;
            return true;
        }
        ...
        if (modelIdToSet !== currentModelId) {
            if (modelIdToSet) {
                ...
                if (result.success) {
                    console.log(chalk.blue(`Set ${role} model: ${result.data.provider} / ${result.data.modelId}`));
                    ...
                    console.error(chalk.red(`Error setting ${role} model: ${result.error?.message || 'Unknown'}`));
                    setupSuccess = false;
                }
            }
    ...
    if (!(await handleSetModel('main', answers.mainModel, currentModels.main?.modelId // <--- Now 'currentModels' is defined
    ))) {
        return false; // Explicitly return false if cancelled
    }
    if (!(await handleSetModel('research', answers.researchModel, currentModels.research?.modelId // <--- Now 'currentModels' is defined
    ))) {
        return false; // Explicitly return false if cancelled
    }
    if (!(await handleSetModel('fallback', answers.fallbackModel, currentModels.fallback?.modelId // <--- Now 'currentModels' is defined
    ))) {
        return false; // Explicitly return false if cancelled
    }
...
function registerCommands(programInstance) {
    ...
    programInstance.on('option:unknown', function (unknownOption) {
        ...
        console.error(chalk.yellow(`Run 'vibex-task-manager ${commandName} --help' to see available options`));
        process.exit(1);
    });
    // parse-prd command
    programInstance
        ...
        .option('-o, --output <file>', 'Output file path', TASKMANAGER_TASKS_FILE)
        .option('-n, --num-tasks <number>', 'Number of tasks to generate', '10')
        .option('-f, --force', 'Skip confirmation when overwriting existing tasks')
        .option('--append', 'Append new tasks to existing tasks.json instead of overwriting')
        .option('-r, --research', 'Use Perplexity AI for research-backed task generation, providing more comprehensive and accurate task breakdown')
        ...
        try {
            if (!inputFile) {
                if (fs.existsSync(defaultPrdPath)) {
                    console.log(chalk.blue(`Using default PRD file path: ${defaultPrdPath}`));
                    if (!(await confirmOverwriteIfNeeded()))
                        return;
                ...
                console.log(chalk.yellow(`No PRD file specified and default PRD file not found at ${PRD_FILE}.`));
                console.log(boxen(`${chalk.white.bold('Parse PRD Help')}\n\n${chalk.cyan('Usage:')}\n  vibex-task-manager parse-prd <prd-file.txt> [options]\n\n${chalk.cyan('Options:')}\n  -i, --input <file>       Path to the PRD file (alternative to positional argument)\n  -o, --output <file>      Output file path (default: "${TASKMANAGER_TASKS_FILE}")\n  -n, --num-tasks <number> Number of tasks to generate (default: 10)\n  -f, --force              Skip confirmation when overwriting existing tasks\n  --append                 Append new tasks to existing tasks.json instead of overwriting\n  -r, --research           Use Perplexity AI for research-backed task generation\n\n${chalk.cyan('Example:')}\n  vibex-task-manager parse-prd requirements.txt --num-tasks 15\n  vibex-task-manager parse-prd --input=requirements.txt\n  vibex-task-manager parse-prd --force\n  vibex-task-manager parse-prd requirements_v2.txt --append\n  vibex-task-manager parse-prd requirements.txt --research\n\n${chalk.yellow('Note: This command will:')}\n  1. Look for a PRD file at ${PRD_FILE} by default\n  2. Use the file specified by --input or positional argument if provided\n  3. Generate tasks from the PRD and either:\n     - Overwrite any existing tasks.json file (default)\n     - Append to existing tasks.json if --append is used`, { padding: 1, borderColor: 'blue', borderStyle: 'round' }));
                return;
            }
            if (!fs.existsSync(inputFile)) {
                console.error(chalk.red(`Error: Input PRD file not found: ${inputFile}`));
                process.exit(1);
            }
            if (!(await confirmOverwriteIfNeeded()))
                return;
    ...
    programInstance
        ...
        .option('-f, --file <file>', 'Path to the tasks file', TASKMANAGER_TASKS_FILE)
        ...
        const tasksPath = options.file || TASKMANAGER_TASKS_FILE;
        ...
        if (process.argv.includes('--id') ||
            ...
            console.log(`  vibex-task-manager update --from=${fromId} --prompt="Your prompt here"`);
            console.log(chalk.yellow('\nTo update a single specific task, use the update-task command instead:'));
            console.log(`  vibex-task-manager update-task --id=<id> --prompt="Your prompt here"`);
            process.exit(1);
        }
        if (!prompt) {
        ...
        await updateTasks(tasksPath, fromId, prompt, useResearch, {} // Pass empty context
        );
    });
    // update-task command
    programInstance
        ...
        try {
            ...
            if (!options.id) {
                ...
                console.log(chalk.yellow('Usage example: vibex-task-manager update-task --id=23 --prompt="Update with new information"'));
                process.exit(1);
            }
            ...
            if (!fs.existsSync(tasksPath)) {
                ...
                if (tasksPath === TASKMANAGER_TASKS_FILE) {
                    console.log(chalk.yellow('Hint: Run vibex-task-manager init or vibex-task-manager parse-prd to create tasks.json first'));
                }
                else {
            ...
            if (error.message.includes('task') &&
                ...
                console.log(chalk.yellow('\nTo fix this issue:'));
                console.log('  1. Run vibex-task-manager list to see all available task IDs');
    ...
    programInstance
        ...
        try {
            ...
            if (!options.id) {
                console.error(chalk.red('Error: --id parameter is required'));
                console.log(chalk.yellow('Usage example: vibex-task-manager update-subtask --id=5.2 --prompt="Add more details about the API endpoint"'));
                process.exit(1);
            }
            ...
            if (error.message.includes('subtask') &&
                error.message.includes('not found')) {
                console.log(chalk.yellow('\nTo fix this issue:'));
                console.log('  1. Run vibex-task-manager list --with-subtasks to see all available subtask IDs');
    ...
    programInstance
        ...
        if (!isValidTaskStatus(status)) {
            console.error(chalk.red(`Error: Invalid status value: ${status}. Use one of: ${TASK_STATUS_OPTIONS.join(', ')}`));
            process.exit(1);
        }
    ...
    programInstance
        ...
        .option('-n, --num <number>', 'Number of subtasks to generate (uses complexity analysis by default if available)')
        ...
        .option('--file <file>', 'Path to the tasks file (relative to project root)', TASKMANAGER_TASKS_FILE // Allow file override
    ) // Allow file override
        .action(async (options) => {
        ...
        if (options.all) {
            ...
            try {
                const result = await expandAllTasks(tasksPath, options.num, // Pass num
            ...
            try {
                ...
                await expandTask(tasksPath, options.id, options.num, options.research, options.prompt, {}, // Pass empty context for CLI calls
    ...
    programInstance
        ...
        .option('-o, --output <file>', 'Output file path for the report', COMPLEXITY_REPORT_FILE)
        ...
        .option('-i, --id <ids>', 'Comma-separated list of specific task IDs to analyze (e.g., "1,3,5")')
    ...
    programInstance
        ...
        const tasksPath = options.file || TASKMANAGER_TASKS_FILE;
        if (!fs.existsSync(tasksPath)) {
            console.error(`❌ No tasks.json file found. Please run "vibex-task-manager init" or create a tasks.json file at ${TASKMANAGER_TASKS_FILE}`);
            process.exit(1);
        }
        ...
        try {
            const { newTaskId, telemetryData } = await addTask(tasksPath, options.prompt, dependenciesArray, options.priority, context, 'text', manualTaskData, options.research);
    ...
    programInstance
        ...
        .option('-f, --file <file>', 'Path to the tasks file', TASKMANAGER_TASKS_FILE)
        .action(async (options) => {
        await validateDependenciesCommand(options.file || TASKMANAGER_TASKS_FILE);
    });
    ...
    programInstance
        ...
        .option('-f, --file <file>', 'Path to the tasks file', TASKMANAGER_TASKS_FILE)
        .action(async (options) => {
        await fixDependenciesCommand(options.file || TASKMANAGER_TASKS_FILE);
    });
    ...
    programInstance
        ...
        try {
            if (existingTaskId) {
                ...
                console.log(boxen(chalk.white.bold(`Subtask ${parentId}.${subtask.id} Added Successfully`) +
                    '\n\n' +
                    ...
                    '\n' +
                    chalk.cyan(`1. Run ${chalk.yellow(`vibex-task-manager show ${parentId}`)} to see the parent task with all subtasks`) +
                    '\n' +
                    chalk.cyan(`2. Run ${chalk.yellow(`vibex-task-manager set-status --id=${parentId}.${subtask.id} --status=in-progress`)} to start working on it`), {
                    padding: 1,
                    borderColor: 'green',
                    borderStyle: 'round',
                    margin: { top: 1 }
                ...
                console.log(boxen(chalk.white.bold('Usage Examples:') +
                    '\n\n' +
                    chalk.white('Convert existing task to subtask:') +
                    '\n' +
                    chalk.yellow(`  vibex-task-manager add-subtask --parent=5 --task-id=8`) +
                    '\n\n' +
                    chalk.white('Create new subtask:') +
                    '\n' +
                    chalk.yellow(`  vibex-task-manager add-subtask --parent=5 --title="Implement login UI" --description="Create the login form"`) +
    ...
    function showAddSubtaskHelp() {
        console.log(boxen(`${chalk.white.bold('Add Subtask Command Help')}\n\n${chalk.cyan('Usage:')}\n  vibex-task-manager add-subtask --parent=<id> [options]\n\n${chalk.cyan('Options:')}\n  -p, --parent <id>         Parent task ID (required)\n  -i, --task-id <id>        Existing task ID to convert to subtask\n  -t, --title <title>       Title for the new subtask\n  -d, --description <text>  Description for the new subtask\n  --details <text>          Implementation details for the new subtask\n  --dependencies <ids>      Comma-separated list of dependency IDs\n  -s, --status <status>     Status for the new subtask (default: "pending")\n  -f, --file <file>         Path to the tasks file (default: "${TASKMANAGER_TASKS_FILE}")\n  --skip-generate           Skip regenerating task files\n\n${chalk.cyan('Examples:')}\n  vibex-task-manager add-subtask --parent=5 --task-id=8\n  vibex-task-manager add-subtask -p 5 -t "Implement login UI" -d "Create the login form"`, { padding: 1, borderColor: 'blue', borderStyle: 'round' }));
    }
        ...
        try {
            ...
            for (const subtaskId of subtaskIdArray) {
                ...
                if (convertToTask && result) {
                    ...
                    console.log(boxen(chalk.white.bold(`Subtask ${subtaskId} Converted to Task #${result.id}`) +
                        '\n\n' +
                        ...
                        chalk.white.bold('Next Steps:') +
                        '\n' +
                        chalk.cyan(`1. Run ${chalk.yellow(`vibex-task-manager show ${result.id}`)} to see details of the new task`) +
                        '\n' +
                        chalk.cyan(`2. Run ${chalk.yellow(`vibex-task-manager set-status --id=${result.id} --status=in-progress`)} to start working on it`), {
                        padding: 1,
                        borderColor: 'green',
                        borderStyle: 'round',
                        margin: { top: 1 }
                    ...
                    console.log(boxen(chalk.white.bold(`Subtask ${subtaskId} Removed`) +
                        '\n\n' +
    ...
    function showRemoveSubtaskHelp() {
        console.log(boxen(chalk.white.bold('Remove Subtask Command Help') +
            '\n\n' +
            chalk.cyan('Usage:') +
            '\n' +
            `  vibex-task-manager remove-subtask --id=<parentId.subtaskId> [options]\n\n` +
            chalk.cyan('Options:') +
            '\n' +
            ...
            chalk.cyan('Examples:') +
            '\n' +
            '  vibex-task-manager remove-subtask --id=5.2\n' +
            '  vibex-task-manager remove-subtask --id=5.2,6.3,7.1\n' +
            '  vibex-task-manager remove-subtask --id=5.2 --convert', { padding: 1, borderColor: 'blue', borderStyle: 'round' }));
    }
    // remove-task command
    programInstance
        .command('remove-task')
        ...
        .description('Remove one or more tasks or subtasks permanently')
        .option('-i, --id <ids>', 'ID(s) of the task(s) or subtask(s) to remove (e.g., "5", "5.2", or "5,6.1,7")')
        .option('-f, --file <file>', 'Path to the tasks file', TASKMANAGER_TASKS_FILE)
        ...
        if (!taskIdsString) {
            ...
            console.error(chalk.yellow('Usage: vibex-task-manager remove-task --id=<taskId1,taskId2...>'));
            process.exit(1);
        }
        ...
        try {
                ...
                if (!taskExists(data.tasks, taskId)) {
                    ...
                    if (taskObject) {
                        ...
                        if (!taskObject.isSubtask) {
                            ...
                            const dependentTasks = data.tasks.filter((t) => t.dependencies &&
                                t.dependencies.includes(parseInt(taskId, 10)));
                            if (dependentTasks.length > 0) {
                                dependentTaskMessages.push(`  - Task ${taskId}: ${dependentTasks.length} dependent tasks (${dependentTasks.map((t) => t.id).join(', ')})`);
                            }
                        }
                    }
                    else {
            ...
            if (nonExistentIds.length > 0) {
                console.warn(chalk.yellow(`Warning: The following task IDs were not found: ${nonExistentIds.join(', ')}`));
            }
            ...
            if (!options.yes) {
                console.log();
                console.log(chalk.red.bold(`⚠️ WARNING: This will permanently delete the following ${existingTasksToRemove.length} item(s):`));
                console.log();
                existingTasksToRemove.forEach(({ id, task }) => {
                    if (!task)
                        return; // Should not happen due to taskExists check, but safeguard
                    if (task.isSubtask) {
                        ...
                        if (task.parentTask) {
                            console.log(chalk.gray(`    (Parent: ${task.parentTask.id} - ${task.parentTask.title || '(no title)'})`));
                        }
                    }
                    else {
                ...
                if (totalSubtasksToDelete > 0) {
                    console.log(chalk.yellow(`⚠️ This will also delete ${totalSubtasksToDelete} subtasks associated with the selected main tasks!`));
                }
                if (dependentTaskMessages.length > 0) {
                    console.log(chalk.yellow('⚠️ Warning: Dependencies on the following tasks will be removed:'));
                ...
                const { confirm } = await inquirer.prompt([
                    {
                        type: 'confirm',
                        name: 'confirm',
                        message: chalk.red.bold(`Are you sure you want to permanently delete these ${existingTasksToRemove.length} item(s)?`),
                        default: false
                    }
                ]);
                if (!confirm) {
            ...
            const indicator = startLoadingIndicator(`Removing ${existingTasksToRemove.length} task(s)/subtask(s)...`);
            ...
            if (result.success) {
                console.log(boxen(chalk.green(`Successfully removed ${result.removedTasks.length} task(s)/subtask(s).`) +
                ...
                console.error(boxen(chalk.red(`Operation completed with errors. Removed ${result.removedTasks.length} task(s)/subtask(s).`) +
                    ('message' in result && result.message ? `\n\nDetails:\n${result.message}` : '') +
            ...
            if (nonExistentIds.length > 0) {
                console.warn(chalk.yellow(`Note: The following IDs were not found initially and were skipped: ${nonExistentIds.join(', ')}`));
    ...
    programInstance
        .command('init')
        .description('Initialize a new project with Task Manager structure')
    ...
    programInstance
        ...
        .addHelpText('after', `
Examples:
  $ vibex-task-manager models                              # View current configuration
  $ vibex-task-manager models --set-main gpt-4o             # Set main model (provider inferred)
  $ vibex-task-manager models --set-research sonar-pro       # Set research model
  $ vibex-task-manager models --set-fallback claude-3-5-sonnet-20241022 # Set fallback
  $ vibex-task-manager models --set-main my-custom-model --ollama  # Set custom Ollama model for main role
  $ vibex-task-manager models --set-main anthropic.claude-3-sonnet-20240229-v1:0 --bedrock # Set custom Bedrock model for main role
  $ vibex-task-manager models --set-main some/other-model --openrouter # Set custom OpenRouter model for main role
  $ vibex-task-manager models --setup                            # Run interactive setup`)
        .action(async (options) => {
        const projectRoot = findProjectRoot();
        if (!projectRoot) {
            console.error(chalk.red('Error: Could not find project root.'));
        ...
        if (providerFlags > 1) {
            console.error(chalk.red('Error: Cannot use multiple provider flags (--openrouter, --ollama, --bedrock) simultaneously.'));
            process.exit(1);
        }
        ...
        if (apiKeyStatusResult.success) {
            ...
            console.error(chalk.yellow(`⚠️ Warning: Could not display API Key status: ${apiKeyStatusResult.error.message}`));
        }
        ...
        if (availableResult.success) {
            const activeIds = configResult.success
                ...
                : [];
            const displayableAvailable = availableResult.data.models.filter((m) => !activeIds.includes(m.modelId) && !m.modelId.startsWith('['));
            displayAvailableModels(displayableAvailable);
        }
        else {
            console.error(chalk.yellow(`⚠️ Warning: Could not display available models: ${availableResult.error.message}`));
        }
        // 4. Conditional Hint if Config File is Missing
        const configExists = isConfigFilePresent(projectRoot);
        if (!configExists) {
            console.log(chalk.yellow("\\nHint: Run 'vibex-task-manager models --setup' to create or update your configuration."));
        }
    ...
    programInstance
        ...
        if (!sourceId || !destinationId) {
            ...
            console.log(chalk.yellow('Usage: vibex-task-manager move --from=<sourceId> --to=<destinationId>'));
            process.exit(1);
        }
        ...
        if (sourceIds.length !== destinationIds.length) {
            ...
            console.log(chalk.yellow('Example: vibex-task-manager move --from=5,6,7 --to=10,11,12'));
            process.exit(1);
        }
        // If moving multiple tasks
        if (sourceIds.length > 1) {
            console.log(chalk.blue(`Moving multiple tasks: ${sourceIds.join(', ')} to ${destinationIds.join(', ')}...`));
            try {
            ...
            try {
                ...
                console.log(chalk.green(`✓ Successfully moved task/subtask ${sourceId} to ${destinationId}`));
            }
            catch (error) {
                console.error(chalk.red(`Error: ${error.message}`));
                process.exit(1);
    ...
    programInstance
        ...
        const projectRoot = findProjectRoot();
        if (!projectRoot) {
            console.error(chalk.red('Error: Could not find project root. Make sure you are in a Task Manager project directory.'));
            process.exit(1);
        }
        console.log(chalk.blue(`📝 Syncing tasks to README.md${withSubtasks ? ' (with subtasks)' : ''}${status ? ` (status: ${status})` : ''}...`));
...
function displayUpgradeNotification(currentVersion, latestVersion) {
    const message = boxen(`${chalk.blue.bold('Update Available!')} ${chalk.dim(currentVersion)} → ${chalk.green(latestVersion)}\n\n` +
        `Run ${chalk.cyan('npm i vibex-task-manager-ai@latest -g')} to update to the latest version with new features and bug fixes.`, {
        padding: 1,
...
async function runCLI(argv = process.argv) {
    try {
        ...
        if (updateInfo && typeof updateInfo === 'object' && 'needsUpdate' in updateInfo && updateInfo.needsUpdate) {
        ...
        if (error instanceof ConfigurationError) {
            console.error(boxen(chalk.red.bold('Configuration Update Required!') +
                ...
                chalk.cyan('`vibex-task-manager models` to check your config & available models\n') +
                chalk.cyan('`vibex-task-manager models --setup` to adjust the AI models used by Taskmaster'), {
                padding: 1,
                margin: { top: 1 },
                borderColor: 'red',
                borderStyle: 'round'
