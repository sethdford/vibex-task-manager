...
async function removeDependency(tasksPath, taskId, dependencyId) {
    ...
    const dependencyIndex = targetTask.dependencies.findIndex((dep) => {
        ...
        if (typeof dep === 'number' && dep < 100 && isSubtask) {
            ...
            const [parentId] = String(formattedTaskId).split('.');
            depStr = `${parentId}.${dep}`;
        }
    ...
    if (!isSilentMode()) {
        // Display a more visually appealing success message
        console.log(boxen(chalk.green(`Successfully removed dependency:\n\n`) +
...
async function validateDependenciesCommand(tasksPath, options = {}) {
    ...
    try {
        ...
        if (!validationResult.valid) {
            log('error', `Dependency validation failed. Found ${validationResult.issues.length} issue(s):`);
            ...
            if (!isSilentMode()) {
                console.log(boxen(chalk.red(`Dependency Validation FAILED\n\n`) +
    ...
    try {
        ...
        if (!isSilentMode()) {
            if (totalFixedAll > 0) {
                ...
                console.log(boxen(chalk.green(`Dependency Fixes Summary:\n\n`) +
...
function ensureAtLeastOneIndependentSubtask(tasksData) {
    ...
    tasksData.tasks.forEach((task) => {
        ...
        const hasIndependentSubtask = task.subtasks.some((st) => !st.dependencies ||
...
export { addDependency, removeDependency, isCircularDependency, validateTaskDependencies, validateDependenciesCommand, fixDependenciesCommand, removeDuplicateDependencies, cleanupSubtaskDependencies, ensureAtLeastOneIndependentSubtask, validateAndFixDependencies };
