...
function parseSubtasksFromText(text, startId, expectedCount, parentTaskId, logger) {
    ...
    if (primaryParseAttemptFailed || !parsedObject) {
        ...
        logger.debug(`Advanced extraction: JSON string that will be parsed: ${jsonToParse.substring(0, 500)}...`);
        try {
            ...
            logger.error(`Advanced extraction: Problematic JSON string for parse (first 500 chars): ${jsonToParse.substring(0, 500)}`);
            throw new Error(
    ...
    if (!parsedObject ||
        typeof parsedObject !== 'object' ||
        !Array.isArray(parsedObject.subtasks)) {
        logger.error(`Final parsed content is not an object or missing 'subtasks' array. Content: ${JSON.stringify(parsedObject).substring(0, 200)}`);
        throw new Error('Parsed AI response is not a valid object containing a "subtasks" array after all attempts.');
    }
    ...
    for (const rawSubtask of parsedSubtasks) {
        const correctedSubtask = {
            ...
            dependencies: Array.isArray(rawSubtask.dependencies)
                ? rawSubtask.dependencies
                    ...
                    .filter((depId) => !isNaN(depId) && depId >= startId && depId < currentId)
                : [],
            status: 'pending'
        };
        ...
        if (result.success) {
            ...
            logger.warn(`Subtask validation failed for raw data: ${JSON.stringify(rawSubtask).substring(0, 100)}...`);
...
async function expandTask(tasksPath, taskId, numSubtasks, useResearch = false, additionalContext = '', context = {}, force = false) {
    ...
    const projectRoot = contextProjectRoot || path.dirname(path.dirname(tasksPath));
    // Use mcpLog if available, otherwise use the default console log wrapper
    const logger = mcpLog || {
        info: (msg) => !isSilentMode() && log('info', msg),
        warn: (msg) => !isSilentMode() && log('warn', msg),
        error: (msg) => !isSilentMode() && log('error', msg),
        debug: (msg) => !isSilentMode() && getDebugFlag(projectRoot) && log('debug', msg) // Use getDebugFlag
    };
    if (mcpLog) {
    ...
    try {
        ...
        if (force && Array.isArray(task.subtasks) && task.subtasks.length > 0) {
            logger.info(`Force flag set. Clearing existing ${task.subtasks.length} subtasks for task ${taskId}.`);
        ...
        try {
            if (fs.existsSync(complexityReportPath)) {
                ...
                if (taskAnalysis) {
                    logger.info(`Found complexity analysis for task ${task.id}: Score ${taskAnalysis.complexityScore}`);
                ...
                logger.info(`Complexity report not found at ${complexityReportPath}. Skipping complexity check.`);
            }
        }
        catch (reportError) {
            logger.warn(`Could not read or parse complexity report: ${reportError.message}. Proceeding without it.`);
        }
        ...
        if (!isNaN(explicitNumSubtasks) && explicitNumSubtasks > 0) {
            ...
            finalSubtaskCount = getDefaultSubtasks(projectRoot);
        ...
        if (taskAnalysis?.expansionPrompt) {
            ...
            logger.info(`Using expansion prompt from complexity report and simplified system prompt for task ${task.id}.`);
            // --- End Simplified System Prompt ---
        }
        else {
            // Use standard prompt generation
            const combinedAdditionalContext = `${additionalContext}${complexityReasoningContext}`.trim();
            if (useResearch) {
                promptContent = generateResearchUserPrompt(task, finalSubtaskCount, combinedAdditionalContext, nextSubtaskId);
                ...
                promptContent = generateMainUserPrompt(task, finalSubtaskCount, combinedAdditionalContext, nextSubtaskId);
        ...
        if (outputFormat === 'text') {
            loadingIndicator = startLoadingIndicator(`Generating ${finalSubtaskCount} subtasks...\n`);
        }
        let responseText = '';
        ...
        try {
            ...
            // Parse Subtasks
            generatedSubtasks = parseSubtasksFromText(responseText, nextSubtaskId, finalSubtaskCount, task.id, logger);
            logger.info(`Successfully parsed ${generatedSubtasks.length} subtasks from AI response.`);
        }
        catch (error) {
            if (loadingIndicator)
                stopLoadingIndicator(loadingIndicator);
            logger.error(`Error during AI call or parsing for task ${taskId}: ${error.message}`, // Added task ID context
            'error');
            // Log raw response in debug mode if parsing failed
            if (error.message.includes('Failed to parse valid subtasks') &&
                getDebugFlag(projectRoot)) {
