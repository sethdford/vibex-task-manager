...
async function addTask(
	...
	context: TaskContext = {},
	outputFormat: OutputFormat = 'text', // Default to text for CLI
	manualTaskData: ManualTaskData | null = null,
	useResearch: boolean = false
): Promise<{ newTaskId: number; telemetryData: any }> {
	...
	const report = (message: string, level: 'debug' | 'info' | 'warn' | 'error' | 'success' = 'info') => {
		if (mcpLog) {
		...
		if (manualTaskData) {
			...
			if (numericDependencies.length > 0) {
				...
				for (const wordResult of wordResults) {
					if (!mergedResults.some((r) => (r.item as any).id === (wordResult.item as any).id)) {
				...
				for (const task of mediumRelevance) {
					if (!allRelevantTasks.some((t) => (t as any).id === (task as any).id)) {
				...
				if (
					...
					contextTasks += `\n\nRecently created tasks:\n${recentTasks
						.filter((t: any) => !relatedTasks.some((rt: any) => rt.id === t.id))
						.slice(0, 3)
						.map((t: any) => `- Task ${t.id}: ${t.title} - ${t.description}`)
						.join('\n')}`;
				}
				...
				uniqueDetailedTasks = Array.from(
					new Map(allDetailedTasks.map((t: any) => [t.id, t])).values()
				).slice(0, 20);
				...
				if (outputFormat === 'text') {
					...
					if (highRelevance.length > 0) {
						...
						highRelevance.slice(0, 25).forEach((t: any) => {
							console.log(
								chalk.yellow(`  • ⭐ Task ${t.id}: ${truncate(t.title, 50)}`)
							);
						});
					...
					if (mediumRelevance.length > 0) {
						...
						mediumRelevance.slice(0, 10).forEach((t: any) => {
							console.log(
					...
					if (uniqueDetailedTasks.length > 0) {
						...
						uniqueDetailedTasks.forEach((t: any) => {
							const isHighRelevance = highRelevance.some(
								(ht: any) => ht.id === (t as any).id
							);
			...
			console.log(
				boxen(
					chalk.white.bold(`Task ${newTaskId} Created Successfully`) +
						...
						'\n' +
						chalk.cyan(
							`1. Run ${chalk.yellow(`vibex-task-manager show ${newTaskId}`)} to see complete task details`
						) +
						'\n' +
						chalk.cyan(
							`2. Run ${chalk.yellow(`vibex-task-manager set-status --id=${newTaskId} --status=in-progress`)} to start working on it`
						) +
						'\n' +
						chalk.cyan(
							`3. Run ${chalk.yellow(`vibex-task-manager expand --id=${newTaskId}`)} to break it down into subtasks`
						),
