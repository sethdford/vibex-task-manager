...
import {
	...
	readJSON,
	truncate,
	readComplexityReport as readComplexityReportUtil,
	addComplexityToTask as addComplexityToTaskUtil
} from '../utils.js';
...
const readComplexityReport = (
	reportPath: string | null
): ComplexityReport | null => {
	if (!reportPath) return null;
	try {
		return readComplexityReportUtil(reportPath) as any;
	} catch (error) {
...
function listTasks(
	...
	try {
		...
		const taskStatusBreakdown: StatusBreakdown = {
			done: completedTasks,
		...
		let totalSubtasks = 0;
		const subtaskStatusBreakdown: StatusBreakdown = {
			done: 0,
			inProgress: 0,
			pending: 0,
			blocked: 0,
		...
		data.tasks.forEach((task) => {
			if (task.subtasks) {
				...
				task.subtasks.forEach((st) => {
					if (st.status === 'done') subtaskStatusBreakdown.done++;
					else if (st.status === 'in-progress')
						subtaskStatusBreakdown.inProgress++;
					else if (st.status === 'pending') subtaskStatusBreakdown.pending++;
					else if (st.status === 'blocked') subtaskStatusBreakdown.blocked++;
					else if (st.status === 'deferred') subtaskStatusBreakdown.deferred++;
		...
		const tasksReadyToWork = data.tasks.filter(
			(t) =>
				t.status !== 'done' &&
				(!t.dependencies ||
		...
		let mostDependedOnTaskDetails: Task | null = null;
		if (Object.keys(dependencyCount).length > 0) {
			const mostDependedOnId = Object.keys(dependencyCount).reduce((a, b) =>
				dependencyCount[a] > dependencyCount[b] ? a : b
			);
				...
				stats: {
					...
					dependencyStats: {
						...
						avgDependenciesPerTask:
							totalTasks > 0 ? totalDependencies / totalTasks : 0
					}
				},
				nextTask: nextItem
			};
		...
		if (outputFormat === 'text') {
			...
			filteredTasks.forEach((task) => {
				const deps = formatDependenciesWithStatus(
					task.dependencies || [],
					data.tasks,
					false
				);
				...
				if (withSubtasks && task.subtasks && task.subtasks.length > 0) {
					task.subtasks.forEach((st) => {
						const subtaskDeps = formatDependenciesWithStatus(
							st.dependencies || [],
							task.subtasks || [],
							true,
							null
			...
			if (nextItem) {
				const nextTaskBox = boxen(
					`${chalk.green('Next Task:')} #${nextItem.id} - ${nextItem.title}`,
					{
						padding: 1,
						margin: { top: 1 },
						borderColor: 'green',
