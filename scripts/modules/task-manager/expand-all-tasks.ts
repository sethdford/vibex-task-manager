...
async function expandAllTasks(
	tasksPath: string,
	numSubtasks: number, // Keep this signature, expandTask handles defaults
	useResearch = false,
	additionalContext = '',
	force = false, // Keep force here for the filter logic
	context: ExpandAllTasksContext = {},
	outputFormat = 'text' // Assume text default for CLI
): Promise<ExpandAllTasksResult> {
	...
	const logger =
		...
		(outputFormat === 'json'
			...
			: {
					...
					debug: (msg) =>
						!isSilentMode() && getDebugFlag(null) && log('debug', msg)
				});
