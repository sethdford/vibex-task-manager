...
async function removeTask(tasksPath, taskIds) {
    ...
    try {
            ...
            try {
                ...
                if (typeof taskId === 'string' && taskId.includes('.')) {
                    ...
                    if (!parentTask || !parentTask.subtasks) {
                        throw new Error(`Parent task ${parentTaskId} or its subtasks not found for subtask ${taskId}`);
                    }
        ...
        if (results.removedTasks.length > 0) {
            ...
            const allRemovedIds = new Set(taskIdsToRemove.map((id) => typeof id === 'string' && id.includes('.') ? id : parseInt(id, 10)));
                ...
                if (task.subtasks) {
                    task.subtasks.forEach((subtask) => {
                        if (subtask.dependencies) {
                            subtask.dependencies = subtask.dependencies.filter((depId) => !allRemovedIds.has(`${task.id}.${depId}`) &&
