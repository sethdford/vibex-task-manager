 ...
 */
async function moveTask(tasksPath, sourceId, destinationId, generateFiles = true) {
    try {
        ...
        if (isSourceSubtask) {
            ...
            sourceSubtaskIndex = sourceParentTask.subtasks.findIndex((st) => st.id === subtaskIdNum);
        ...
        if (isDestinationSubtask) {
            ...
            destSubtaskIndex = destParentTask.subtasks.findIndex((st) => st.id === subtaskIdNum);
            ...
            if (destTaskIndex === -1) {
                ...
                let insertIndex = 0;
                while (insertIndex < data.tasks.length &&
        ...
        if (!isSourceSubtask && !isDestinationSubtask) {
            ...
            if (isTaskDependentOn(data.tasks, destTask, sourceIdNum)) {
                throw new Error(`Cannot move task ${sourceId} to task ${destinationId} as it would create a circular dependency`);
            }
        }
        let movedTask;
        ...
        if (!isSourceSubtask && !isDestinationSubtask) {
            ...
            if (sourceParentId === destParentId) {
                ...
                movedTask = moveSubtaskToAnotherParent(sourceSubtask, sourceParentTask, sourceSubtaskIndex, destParentTask, destSubtaskIndex);
            }
        }
...
function moveTaskToTask(data, sourceTask, sourceTaskIndex, destTask) {
    ...
    const highestSubtaskId = destTask.subtasks.length > 0
    ...
    log('info', `Moved task ${sourceTask.id} to become subtask ${destTask.id}.${newSubtaskId}`);
    return newSubtask;
}
/**
...
function moveTaskToSubtaskPosition(data, sourceTask, sourceTaskIndex, destParentTask, destSubtaskIndex) {
    ...
    // Find the highest subtask ID to determine the next ID
    const highestSubtaskId = destParentTask.subtasks.length > 0
    ...
    // Remove the original task from the tasks array
    data.tasks.splice(sourceTaskIndex, 1);
    log('info', `Moved task ${sourceTask.id} to become subtask ${destParentTask.id}.${newSubtaskId}`);
    return newSubtask;
}
/**
...
function moveSubtaskToTask(data, sourceSubtask, sourceParentTask, sourceSubtaskIndex, destTask) {
    ...
    log('info', `Moved subtask ${sourceParentTask.id}.${sourceSubtask.id} to become task ${newTaskId}`);
    return newTask;
}
/**
...
function reorderSubtask(parentTask, sourceIndex, destIndex) {
    ...
    log('info', `Reordered subtask ${parentTask.id}.${subtask.id} within parent task ${parentTask.id}`);
    return subtask;
}
/**
 ...
 * @returns {Object} Moved subtask object
 */
function moveSubtaskToAnotherParent(sourceSubtask, sourceParentTask, sourceSubtaskIndex, destParentTask, destSubtaskIndex) {
    ...
    log('info', `Moved subtask ${sourceParentTask.id}.${sourceSubtask.id} to become subtask ${destParentTask.id}.${newSubtaskId}`);
    return newSubtask;
}
/**
