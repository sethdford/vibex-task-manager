...
interface AnalyzeComplexityOptions {
	...
	from?: number;
	to?: number;
	_filteredTasksData?: any;
	_originalTaskCount?: number;
	file?: string;
}

interface AnalyzeComplexityContext {
	session?: any;
	mcpLog?: any;
	reportProgress?: Function;
}

async function analyzeTaskComplexity(options: AnalyzeComplexityOptions, context: AnalyzeComplexityContext = {}) {
		...
		try {
			...
			if (outputFormat === 'text') {
				...
				if (aiError.message.includes('API key')) {
					...
					console.log(
						chalk.yellow("Run 'vibex-task-manager models --setup' if needed.")
					);
				}
			}
			throw aiError;
