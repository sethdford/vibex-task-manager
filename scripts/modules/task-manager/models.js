...
import { getMainModelId, getResearchModelId, getFallbackModelId, getAvailableModels, getMainProvider, getResearchProvider, getFallbackProvider, isApiKeySet, getMcpApiKeyStatus, getConfig, writeConfig, isConfigFilePresent, getAllProviders } from '../config-manager.js';
...
async function getModelConfiguration(options = {}) {
    ...
    if (!configExists) {
        throw new Error('The configuration file is missing. Run "vibex-task-manager models --setup" to create it.');
    }
    try {
    ...
    try {
        ...
        if (providerHint) {
            ...
            if (modelData && modelData.provider === providerHint) {
                ...
                report('info', `Model ${modelId} found internally with matching provider hint ${determinedProvider}.`);
            }
            else {
                ...
                if (providerHint === 'openrouter') {
                    ...
                    if (openRouterModels &&
                        ...
                        throw new Error(`Model ID "${modelId}" not found in the live OpenRouter model list. Please verify the ID and ensure it's available on OpenRouter.`);
                    }
                }
