...
import { log, readJSON, truncate, readComplexityReport, addComplexityToTask } from '../utils.js';
import findNextTask from './find-next-task.js';
import { getStatusWithColor, formatDependenciesWithStatus, getComplexityWithColor, createProgressBar } from '../ui.js';
/**
 * List all tasks
...
function listTasks(tasksPath, statusFilter, reportPath = null, withSubtasks = false, outputFormat = 'text') {
    try {
        ...
        const completedTasks = data.tasks.filter((task) => task.status === 'done' || task.status === 'completed').length;
        ...
        data.tasks.forEach((task) => {
            if (task.subtasks && task.subtasks.length > 0) {
                ...
                completedSubtasks += task.subtasks.filter((st) => st.status === 'done' || st.status === 'completed').length;
        ...
        const subtaskCompletionPercentage = totalSubtasks > 0 ? (completedSubtasks / totalSubtasks) * 100 : 0;
        ...
        const completedTaskIds = new Set(data.tasks
            ...
            .map((t) => t.id));
        const tasksWithNoDeps = data.tasks.filter((t) => t.status !== 'done' &&
            t.status !== 'completed' &&
            (!t.dependencies || t.dependencies.length === 0)).length;
        const tasksWithAllDepsSatisfied = data.tasks.filter((t) => t.status !== 'done' &&
            t.status !== 'completed' &&
            t.dependencies &&
            t.dependencies.length > 0 &&
            t.dependencies.every((depId) => completedTaskIds.has(depId))).length;
        const tasksWithUnsatisfiedDeps = data.tasks.filter((t) => t.status !== 'done' &&
            t.status !== 'completed' &&
            t.dependencies &&
            t.dependencies.length > 0 &&
            !t.dependencies.every((depId) => completedTaskIds.has(depId))).length;
        ...
        for (const [taskId, count] of Object.entries(dependencyCount)) {
            const dependentCount = typeof count === 'number' ? count : 0;
        ...
        const totalDependencies = data.tasks.reduce((sum, task) => sum + (task.dependencies ? task.dependencies.length : 0), 0);
...
` +
        ...
        if (filteredTasks.length === 0) {
            console.log(boxen(statusFilter
                ? chalk.yellow(`No tasks with status '${statusFilter}' found`)
                : chalk.yellow('No tasks found'), { padding: 1, borderColor: 'yellow', borderStyle: 'round' }));
            return;
        }
        ...
        filteredTasks.forEach((task) => {
            ...
            if (task.dependencies && task.dependencies.length > 0) {
                ...
                depText = formatDependenciesWithStatus(task.dependencies, data.tasks, true, complexityReport);
            }
            else {
        ...
        if (nextItem) {
            ...
            console.log(boxen(chalk.hex('#FF8800').bold(
            ...
            `🔥 Next Task to Work On: #${nextItem.id} - ${nextItem.title}`) +
                ...
                '\n\n' +
                // Use nextItem.id
                `${chalk.cyan('Start working:')} ${chalk.yellow(`vibex-task-manager set-status --id=${nextItem.id} --status=in-progress`)}\n` +
                // Use nextItem.id
                `${chalk.cyan('View details:')} ${chalk.yellow(`vibex-task-manager show ${nextItem.id}`)}`, {
            ...
            console.log(boxen(chalk.hex('#FF8800').bold('No eligible next task found') +
                '\n\n' +
        ...
        // Show next steps
        console.log(boxen(chalk.white.bold('Suggested Next Steps:') +
            '\n\n' +
            `${chalk.cyan('1.')} Run ${chalk.yellow('vibex-task-manager next')} to see what to work on next\n` +
            `${chalk.cyan('2.')} Run ${chalk.yellow('vibex-task-manager expand --id=<id>')} to break down a task into subtasks\n` +
            `${chalk.cyan('3.')} Run ${chalk.yellow('vibex-task-manager set-status --id=<id> --status=done')} to mark a task as complete`, {
            padding: 1,
            borderColor: 'gray',
            borderStyle: 'round',
            margin: { top: 1 }
...
function generateMarkdownOutput(data, filteredTasks, stats) {
    ...
    if (nextItem) {
        ...
        markdown += `│  Start working: vibex-task-manager set-status --id=${nextItem.id} --status=in-progress                                                     │\n`;
        markdown += `│  View details: vibex-task-manager show ${nextItem.id}                                                                      │\n`;
        markdown +=
            '│                                                                                                                         │\n';
        markdown +=
            '╰─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╯\n\n';
    ...
    markdown +=
        '│                                                                                      │\n';
    markdown +=
        '│   1. Run vibex-task-manager next to see what to work on next                                │\n';
    markdown +=
        '│   2. Run vibex-task-manager expand --id=<id> to break down a task into subtasks             │\n';
    markdown +=
        '│   3. Run vibex-task-manager set-status --id=<id> --status=done to mark a task as complete   │\n';
    markdown +=
        '│                                                                                      │\n';
    markdown +=
        '╰──────────────────────────────────────────────────────────────────────────────────────╯\n';
