...
import { LEGACY_CONFIG_FILE, TASKMANAGER_CONFIG_FILE } from '../../../src/constants/paths.js';
    ...
    if (fs.existsSync(scriptsDir)) {
        ...
        for (const file of scriptsFiles) {
            ...
            if (fs.statSync(filePath).isFile()) {
                ...
                if (lowerFile.includes('example') ||
                ...
                else if (lowerFile.includes('complexity') &&
                ...
                else if (lowerFile.includes('prd') ||
                    ...
                    log.warn(`Skipping migration of '${file}' - uncertain categorization. You may need to move this manually.`);
                    continue;
                }
                migrationPlan.push({
...
async function performMigration(projectRoot, migrationPlan, options) {
    ...
    for (const dir of neededDirs) {
        const fullDirPath = path.join(projectRoot, String(dir));
    ...
    for (const item of migrationPlan) {
        ...
        // Create backup if requested
        if (options.backup) {
            const backupPath = path.join(projectRoot, '.taskmaster-migration-backup', String(item.from));
