...
async function displayTaskById(
	...
	const originalSubtasks = (taskResult as any).originalSubtasks;
...
function displayApiKeyStatus(statusReport) {
	...
	console.log(
		chalk.gray(
			`  Note: Some providers (e.g., Azure, Ollama) may require additional endpoint configuration in ${TASKMANAGER_CONFIG_FILE}.`
		)
	);
}
	...
	console.log(
		boxen(
			chalk.white.bold('Next Steps:') +
				...
				'\n' +
				chalk.cyan(
					`5. Use custom ollama/openrouter models: ${chalk.yellow('vibex-task-manager models --openrouter|ollama --set-main|research|fallback <model_id>')}`
				),
			{
				padding: 1,
				borderColor: 'yellow',
