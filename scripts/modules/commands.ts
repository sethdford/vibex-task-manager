 ...
 */
async function runInteractiveSetup(projectRoot: string) {
	if (!projectRoot) {
		console.error(
			chalk.red(
	...
	const getPromptData = (role: string, allowNone: boolean = false) => {
		const currentModel = (currentModels as any)[role]; // Use the fetched data
		const allModelsRaw = getAvailableModels(); // Get all available models

		// Manually group models by provider
		const modelsByProvider = allModelsRaw.reduce((acc: any, model: any) => {
		...
		const roleChoices = Object.entries(modelsByProvider)
			.map(([provider, models]: [string, any]) => {
				const providerModels = (models as any[])
					.filter((m: any) => m.allowed_roles.includes(role))
					.map((m: any) => ({
						name: `${provider} / ${m.id} ${
	...
	async function handleSetModel(role: 'main' | 'research' | 'fallback', selectedValue: any, currentModelId: any) {
		...
		if (selectedValue === '__CUSTOM_OPENROUTER__') {
			...
			console.log(
				chalk.blue(
					`Custom OpenRouter model "${modelIdToSet}" will be used. Validation will occur at runtime.`
				)
			);
			...
			console.log(
				chalk.blue(
					`Custom Ollama model "${modelIdToSet}" will be used. Validation will occur at runtime.`
				)
			);
...
function registerCommands(programInstance) {
	...
	programInstance.on('option:unknown', function (unknownOption) {
		...
		console.error(
			chalk.yellow(
				`Run 'vibex-task-manager ${commandName} --help' to see available options`
			)
		);
		process.exit(1);
	});
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (
				...
				console.log(
					`  vibex-task-manager update --from=${fromId} --prompt="Your prompt here"`
				);
				console.log(
					chalk.yellow(
				...
				console.log(
					`  vibex-task-manager update-task --id=<id> --prompt="Your prompt here"`
				);
				process.exit(1);
			}
		...
		.action(async (options) => {
			try {
				...
				if (!options.id) {
					...
					console.log(
						chalk.yellow(
							'Usage example: vibex-task-manager update-task --id=23 --prompt="Update with new information"'
						)
					);
					process.exit(1);
				}
				...
				if (!fs.existsSync(tasksPath)) {
					...
					if (tasksPath === TASKMANAGER_TASKS_FILE) {
						console.log(
							chalk.yellow(
								'Hint: Run vibex-task-manager init or vibex-task-manager parse-prd to create tasks.json first'
							)
						);
					} else {
						console.log(
				...
				if (
					...
					console.log(chalk.yellow('\nTo fix this issue:'));
					console.log(
						'  1. Run vibex-task-manager list to see all available task IDs'
					);
		...
		.action(async (options) => {
			try {
				...
				if (!options.id) {
					...
					console.log(
						chalk.yellow(
							'Usage example: vibex-task-manager update-subtask --id=5.2 --prompt="Add more details about the API endpoint"'
						)
					);
					process.exit(1);
				}
	...
	programInstance
		...
		.option(
			...
			TASKMANAGER_TASKS_FILE // Allow file override
		) // Allow file override
		.action(async (options) => {
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (!fs.existsSync(tasksPath)) {
				console.error(
					`❌ No tasks.json file found. Please run "vibex-task-manager init" or create a tasks.json file at ${TASKMANAGER_TASKS_FILE}`
				);
				process.exit(1);
			}
					...
					console.log(
						boxen(
							chalk.white.bold(
								...
								'\n' +
								chalk.cyan(
									`1. Run ${chalk.yellow(`vibex-task-manager show ${parentId}`)} to see the parent task with all subtasks`
								) +
								'\n' +
								chalk.cyan(
									`2. Run ${chalk.yellow(`vibex-task-manager set-status --id=${parentId}.${subtask.id} --status=in-progress`)} to start working on it`
								),
							{
								padding: 1,
								borderColor: 'green',
					...
					console.log(
						boxen(
							chalk.white.bold('Usage Examples:') +
								...
								'\n' +
								chalk.yellow(
									`  vibex-task-manager add-subtask --parent=5 --title="Implement login UI" --description="Create the login form"`
								) +
								'\n\n',
							{ padding: 1, borderColor: 'blue', borderStyle: 'round' }
						)
						...
						console.log(
							boxen(
								chalk.white.bold(
									...
									'\n' +
									chalk.cyan(
										`1. Run ${chalk.yellow(`vibex-task-manager show ${result.id}`)} to see details of the new task`
									) +
									'\n' +
									chalk.cyan(
										`2. Run ${chalk.yellow(`vibex-task-manager set-status --id=${result.id} --status=in-progress`)} to start working on it`
									),
								{
									padding: 1,
									borderColor: 'green',
		...
		console.log(
			boxen(
				chalk.white.bold('Remove Subtask Command Help') +
					...
					'  vibex-task-manager remove-subtask --id=5.2 --convert',
				{ padding: 1, borderColor: 'blue', borderStyle: 'round' }
			)
		);
	}
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (!taskIdsString) {
				...
				console.error(
					chalk.yellow(
						'Usage: vibex-task-manager remove-task --id=<taskId1,taskId2...>'
					)
				);
				process.exit(1);
			}
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (!sourceId || !destinationId) {
				...
				console.log(
					chalk.yellow(
						'Usage: vibex-task-manager move --from=<sourceId> --to=<destinationId>'
					)
				);
				process.exit(1);
			}
			...
			if (sourceIds.length !== destinationIds.length) {
				...
				console.log(
					chalk.yellow(
						'Example: vibex-task-manager move --from=5,6,7 --to=10,11,12'
					)
				);
				process.exit(1);
			}
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (!projectRoot) {
				console.error(
					chalk.red(
						'Error: Could not find project root. Make sure you are in a Task Manager project directory.'
					)
				);
				process.exit(1);
			}
	...
	try {
		...
		if (updateInfo && typeof updateInfo === 'object' && 'needsUpdate' in updateInfo && updateInfo.needsUpdate) {
			displayUpgradeNotification(
				(updateInfo as any).currentVersion,
				(updateInfo as any).latestVersion
			);
		}
	} catch (error) {
